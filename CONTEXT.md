# Ottili Coder Session Runtime

Ottili Coder sessions preserve durable conversational history while assembling the runtime context an agent needs to act correctly in its current environment.
...
A temporary file created under Ottili Coder's shared tool-output directory to retain complete output that was too large for Session history.
...
- Ambient project instruction discovery honors `OTTILI_CODER_DISABLE_PROJECT_CONFIG`; global instructions remain eligible.
