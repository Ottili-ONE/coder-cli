...
import { createOttiliCoderClient } from "@opencode-ai/sdk"
...
const { client, server } = createOttiliCoder()
...
try {
  ...
  await assertOttiliCoderConnected()
...
function createOttiliCoder() {
  const host = "127.0.0.1"
  const port = 4096
  const url = `http://${host}:${port}`
  const proc = spawn(`ottili-coder`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createOttiliCoderClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/ottili-coder|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/ottili-coder` or `/oc`")
  }
}
...
async function assertOttiliCoderConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to ottili-coder server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.ottili.one/coder" : "https://ottili.one/coder"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.ottili.one/coder/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("ottili-coder-github-action")
    response = await fetch("https://api.ottili.one/coder/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/ottili-coder" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/ottili-coder") || body.includes("/oc")) {
      if (reviewContext) {
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `ottili-coder/${type}${useIssueId()}-${timestamp}`
}
...
Generated by Ottili Coder https://ottili.one/coder"`
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/ottili-coder-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[ottili-coder session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
