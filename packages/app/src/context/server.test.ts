  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://ottiliCoder.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://ottiliCoder.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://ottiliCoder.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://ottiliCoder.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://ottiliCoder.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://ottiliCoder.example.com"),
      ),
    ).toEqual({
