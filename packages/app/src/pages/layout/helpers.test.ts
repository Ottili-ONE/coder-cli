...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("ottiliCoder://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("ottiliCoder://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("ottiliCoder://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("ottiliCoder://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("ottiliCoder://open-project")).toBeUndefined()
    expect(parseDeepLink("ottiliCoder://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "ottiliCoder://open-project?directory=/a",
      "ottiliCoder://other?directory=/b",
      "ottiliCoder://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("ottiliCoder://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("ottiliCoder://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("ottiliCoder://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("ottiliCoder://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "ottiliCoder://new-session?directory=/a",
      "ottiliCoder://open-project?directory=/b",
      "ottiliCoder://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OTTILI_CODER__: {
        deepLinks: ["ottiliCoder://open-project?directory=/a"],
      },
    } as unknown as Window & { __OTTILI_CODER__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["ottiliCoder://open-project?directory=/a"])
