...
export type WslOttiliCoderCheck = {
  distro: string
...
export type WslJob =
  ...
  | { kind: "probe-ottili-coder"; distro: string; startedAt: number }
  | { kind: "install-ottili-coder"; distro: string; startedAt: number }
...
export type WslServersPlatform = {
  ...
  probeOttiliCoder(name: string): Promise<void>
  installOttiliCoder(name: string): Promise<void>
