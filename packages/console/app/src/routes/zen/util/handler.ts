...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-ottili-coder-session") ?? ""
    const requestId = input.request.headers.get("x-ottili-coder-request") ?? ""
    const ocClient = input.request.headers.get("x-ottili-coder-client") ?? ""
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWith429Retry(reqUrl, {
        ...
        headers: (() => {
          ...
          headers.delete("x-ottili-coder-request")
          headers.delete("x-ottili-coder-session")
          headers.delete("x-ottili-coder-project")
          headers.delete("x-ottili-coder-client")
          return headers
        })(),
        body: reqBody,
      })
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://ottili.one/coder/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://ottili.one/coder/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://ottili.one/coder/workspace/${authInfo.workspaceID}/members`
