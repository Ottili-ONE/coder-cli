...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@opencode-ai/acme")).toBe("@opencode-ai/acme")
    expect(Npm.sanitize("@opencode-ai/acme@1.0.0")).toBe("@opencode-ai/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/ottili-coder/acme.git"
    const expected = win ? "acme@git+https_//github.com/ottili-coder/acme.git" : spec
