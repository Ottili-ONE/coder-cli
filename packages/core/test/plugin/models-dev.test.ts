...
describe("ModelsDevPlugin", () => {
  it.effect("registers key connectors for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.OTTILI_CODER_MODELS_PATH,
          disabled: Flag.OTTILI_CODER_DISABLE_MODELS_FETCH,
        }
        Flag.OTTILI_CODER_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.OTTILI_CODER_MODELS_PATH = previous.path
          Flag.OTTILI_CODER_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
