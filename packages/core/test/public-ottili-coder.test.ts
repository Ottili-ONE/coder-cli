...
import { AbsolutePath, Location, Model, OttiliCoder, Session, Tool } from "@opencode-ai/core/public"
import { tmpdir } from "./fixture/tmpdir"
import { testEffect } from "./lib/effect"

const it = testEffect(OttiliCoder.layer)

describe("public native Ottili Coder API", () => {
  it.effect("exposes only the intentional Session capabilities", () =>
    Effect.gen(function* () {
      const ottiliCoder = yield* OttiliCoder.Service

      expect(Object.keys(ottiliCoder).sort()).toEqual(["sessions", "tools"])

      expect(Object.keys(ottiliCoder.sessions).sort()).toEqual([
        "context",
        "create",
        "events",
        "get",
      ...
      expect(yield* ottiliCoder.sessions.list()).toBeArray()
      yield* ottiliCoder.tools.register({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          yield* ottiliCoder.sessions.create({
            id: sessionID,
            location: Location.Ref.make({ directory: AbsolutePath.make(tmp.path) }),
          })

          yield* ottiliCoder.sessions.switchModel({ sessionID, model })

          expect((yield* ottiliCoder.sessions.get(sessionID)).model).toEqual(model)
        }),
      ),
    ),
  )
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap(([available, disabled]) =>
        Effect.gen(function* () {
          ...
          yield* ottiliCoder.sessions.switchModel({ sessionID: availableID, model: ref({ variant: "default" }) })
          const disabledError = yield* ottiliCoder.sessions
            .switchModel({ sessionID: disabledID, model: ref() })
            .pipe(Effect.flip)
          const missingError = yield* ottiliCoder.sessions
          ...
          expect((yield* ottiliCoder.sessions.get(availableID)).model).toEqual(ref({ variant: "default" }))
          expect((yield* ottiliCoder.sessions.get(disabledID)).model).toBeUndefined()
        }),
      ),
    ),
  )
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          yield* ottiliCoder.sessions.switchModel({ sessionID, model: selected })

          const error = yield* ottiliCoder.sessions
            .switchModel({ sessionID, model: ref({ variant: "unknown" }) })
            .pipe(Effect.flip)

          expect(error).toBeInstanceOf(Session.VariantUnavailableError)
          expect((yield* ottiliCoder.sessions.get(sessionID)).model).toEqual(selected)
        }),
      ),
    ),
  )
