...
export async function exportDebugLogs() {
  ...
  const output = join(app.getPath("downloads"), `ottili-coder-debug-${stamp()}.zip`)
  try {
...
function serverLogRoots() {
  ...
  return [...new Set([join(xdgData, "ottili-coder", "log"), join(app.getPath("userData"), "ottili-coder", "log")])]
}
