...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.ottiliCoder.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `ottiliCoder.settings.dat` is special: it maps to the `ottiliCoder.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"ottiliCoder.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // ottiliCoder.settings.dat → the electron settings store ("ottiliCoder.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "ottiliCoder.global.dat").
  const storeName = filename === "ottiliCoder.settings.dat" ? "ottiliCoder.settings" : filename
