...
import { OttiliCoderHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for ottili-coder serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(OttiliCoderHttpApi)
...
export const PublicApi = OttiliCoderHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "ottili-coder",
    version: "1.0.0",
    description: "ottili-coder api",
