...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".ottili-coder")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .ottili-coder/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const ottiliCoder = path.join(test.directory, ".ottili-coder")
      const tool = path.join(ottiliCoder, "tool")
  ...
  it.instance("ignores non-tool exports in .ottili-coder/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".ottili-coder", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-ottili-coder` was registering a tool
  ...
  it.instance("loads tools from .ottili-coder/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const ottiliCoder = path.join(test.directory, ".ottili-coder")
      const tools = path.join(ottiliCoder, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".ottili-coder", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const ottiliCoder = path.join(test.directory, ".ottili-coder")
        const customTools = path.join(ottiliCoder, "tools")
        const plugin = path.join(ottiliCoder, "node_modules", "@opencode-ai", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(ottiliCoder, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".ottili-coder", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(ottiliCoder, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(ottiliCoder, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(ottiliCoder, "node_modules", "cowsay")
