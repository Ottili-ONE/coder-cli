...
Practical reference for a future `packages/server` split after the ottili-coder
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/ottili-coder`.
...
Until enough shared service code lives outside `packages/ottili-coder`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/ottili-coder`

It should not import `packages/ottili-coder` services while `packages/ottili-coder`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on ottili-coder-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/ottili-coder` implementation details.
