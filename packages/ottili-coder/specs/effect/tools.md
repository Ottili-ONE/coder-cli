...
Practical reference for the current tool-migration state in `packages/ottili-coder`.
...
Tool tests should use the existing Effect helpers in `packages/ottili-coder/test/lib/effect.ts`:
