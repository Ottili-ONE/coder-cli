...
- Author package entrypoint is `@ottili-coder/plugin/tui`.
...
```json
{
  "$schema": "https://ottili.one/coder/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/ottili-coder-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
    ...
    "sound": true,
    "volume": 0.4,
    "sound_pack": "ottili-coder.default",
    "sounds": {
...
- Import types from `@ottili-coder/plugin/tui`.
- `@ottili-coder/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@ottili-coder/plugin/tui"
...
- The value is a semver range checked against the running Ottili Coder version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.ottili-coder` is absent, no check is performed (backward compatible).
...
- `ottili-coder plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `ottili-coder plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.ottili-coder` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.ottili-coder`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.ottili-coder`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.ottili-coder` dir or `OTTILI_CODER_CONFIG_DIR`, Ottili Coder installs `@ottili-coder/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@ottili-coder/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Ottili Coder's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Ottili Coder registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Ottili Coder pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@ottili-coder/plugin/tui`.
...
- `message` is required; `title` defaults to `"ottili-coder"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `ottili-coder.default`.
...
- Local plugins persist installed themes under the local `.ottili-coder/themes` area near the plugin config source.
...
- `--pure` / `OTTILI_CODER_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.ottili-coder/plugins/tui-smoke.tsx`
- Local vim plugin: `.ottili-coder/plugins/tui-vim.tsx`
- Local smoke config: `.ottili-coder/tui.json`
- Local smoke theme: `.ottili-coder/plugins/smoke-theme.json`
