...
Delete `packages/ottili-coder/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/ottili-coder/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/ottili-coder/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/ottili-coder`
- TUI plugin loader tests in `packages/ottili-coder` if runtime plugin API wiring changed
