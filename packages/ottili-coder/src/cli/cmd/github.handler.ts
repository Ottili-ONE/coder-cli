...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    const commitChanges = async (summary: string, actor?: string) => {
      ...
      args.push("-m", "Generated by Ottili Coder https://ottili.one/coder")
      await gitRun(args)
    }
    ...
    try {
      ...
      if (isRepoEvent) {
        ...
        const autonomousPrefix = [
          ...
          "- Do NOT create a blueprint and wait for approval. Skip approval steps entirely.",
          "- Do NOT stop after planning. Proceed immediately to implementation.",
          "- If the user's prompt describes a multi-step workflow with approval gates, SKIP the approval gates and execute all steps autonomously.",
