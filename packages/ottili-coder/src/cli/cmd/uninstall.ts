...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall ottili-coder and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Ottili Coder")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g ottili-coder-ai",
      pnpm: "pnpm uninstall -g ottili-coder-ai",
      bun: "bun remove -g ottili-coder-ai",
      yarn: "yarn global remove ottili-coder-ai",
      brew: "brew uninstall ottili-coder",
      choco: "choco uninstall ottili-coder",
      scoop: "scoop uninstall ottili-coder",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "ottili-coder", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".ottili-coder")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Ottili Coder!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# ottili-coder") || content.includes(".ottili-coder/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".ottili-coder/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".ottili-coder/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".ottili-coder"))
    ) {
      continue
    }
