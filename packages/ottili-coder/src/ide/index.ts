...
export function alreadyInstalled() {
  return process.env["OTTILI_CODER_CALLER"] === "vscode" || process.env["OTTILI_CODER_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.ottili-coder"], {
    nothrow: true,
  })
