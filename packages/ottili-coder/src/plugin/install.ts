...
export type PatchDeps = {
  ...
  files: (dir: string, name: "ottiliCoder" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".ottili-coder")
}

function patchName(kind: Kind): "ottiliCoder" | "tui" {
  if (kind === "server") return "ottiliCoder"
  return "tui"
}
