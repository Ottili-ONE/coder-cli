...
export function plan(input: { slug: string; time: { created: number } }, instance: InstanceContext) {
  const base = instance.project.vcs
    ? path.join(instance.worktree, ".ottili-coder", "plans")
    : path.join(Global.Path.data, "plans")
  return path.join(base, [input.time.created, input.slug].join("-") + ".md")
}

export function build(input: { slug: string; time: { created: number } }, instance: InstanceContext) {
  const base = instance.project.vcs
    ? path.join(instance.worktree, ".ottili-coder", "builds")
    : path.join(Global.Path.data, "builds")
...
export function fullrun(input: { slug: string; time: { created: number } }, instance: InstanceContext) {
  const base = instance.project.vcs
    ? path.join(instance.worktree, ".ottili-coder", "runs")
    : path.join(Global.Path.data, "runs")
  return path.join(base, [input.time.created, input.slug].join("-") + ".json")
}

export function taskgraph(input: { slug: string; time: { created: number } }, instance: InstanceContext) {
  const base = instance.project.vcs
    ? path.join(instance.worktree, ".ottili-coder", "taskgraphs")
    : path.join(Global.Path.data, "taskgraphs")
  return path.join(base, [input.time.created, input.slug].join("-") + ".json")
}
...
export class Service extends Context.Service<Service, Interface>()("@opencode-ai/Session") {}
