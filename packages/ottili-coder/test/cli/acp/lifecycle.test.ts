...
describe("ottili-coder acp lifecycle subprocess", () => {
  cliIt.live(
    "stdin EOF exits cleanly",
    ({ ottiliCoder }) =>
      Effect.gen(function* () {
        const acp = yield* ottiliCoder.acp()
        acp.close()
  ...
  cliIt.live(
    ...
    ({ home, llm, ottiliCoder }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient(
          { ottiliCoder },
          { OTTILI_CODER_CONFIG_CONTENT: JSON.stringify(verifierConfig(llm.url)) },
        )
