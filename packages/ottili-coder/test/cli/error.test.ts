...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/ottiliCoder.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/ottiliCoder.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/ottiliCoder.jsonc", dir: ".ottili-coder", suggestion: "ottili-coder" },
        expected:
          'Directory ".ottili-coder" in /tmp/ottiliCoder.jsonc is not valid. Rename the directory to "ottili-coder" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/ottiliCoder.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/ottiliCoder.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.ottili.one/coder/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.ottili.one/coder/auth/device/code.")
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `ottili-coder models` to list available models",
      "Or check your config (ottiliCoder.json) provider/model names",
    ].join("\n")
