...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "ottiliCoder.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "ottiliCoder.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "ottiliCoder.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "ottiliCoder.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["ottiliCoder.json", "ottiliCoder.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "ottiliCoder.json"),
        path.join(project, "ottiliCoder.jsonc"),
      ])
    })
  })
