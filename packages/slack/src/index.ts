...
import { createOttiliCoder, type ToolPart } from "@opencode-ai/sdk"
...
console.log("🚀 Starting ottili-coder server...")
const ottiliCoder = await createOttiliCoder({
  port: 0,
})
console.log("✅ Ottili Coder server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await ottiliCoder.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new ottili-coder session...")
    const { client, server } = ottiliCoder
    ...
    console.log("✅ Created ottili-coder session:", createResult.data.id)
  ...
  console.log("📝 Sending to ottiliCoder:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Ottili Coder response:", JSON.stringify(result, null, 2))
