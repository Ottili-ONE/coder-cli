...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve ottili-coder route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "ottili-coder")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "ottili-coder")).toBe("openai")
    expect(statProvider("big-pickle", "", "ottili-coder")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "ottili-coder"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep ottili-coder as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "ottili-coder"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "ottili-coder"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep ottili-coder or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "ottili-coder"), country: "US" })).toMatchObject([
