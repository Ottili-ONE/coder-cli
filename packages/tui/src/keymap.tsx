...
export const OTTILI_CODER_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const OTTILI_CODER_MODE_KEY = "ottiliCoder.mode"

export const OttiliCoderKeymapProvider = KeymapProvider
export const useOttiliCoderKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type OttiliCoderModeStack = ReturnType<typeof createOttiliCoderModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, OttiliCoderModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createOttiliCoderModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(OTTILI_CODER_MODE_KEY, OTTILI_CODER_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(OTTILI_CODER_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(OTTILI_CODER_MODE_KEY, stack.at(-1)?.mode ?? OTTILI_CODER_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? OTTILI_CODER_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(OTTILI_CODER_MODE_KEY, undefined)
...
export function useOttiliCoderModeStack() {
  return getOttiliCoderModeStack(useOttiliCoderKeymap())
}

export function getOttiliCoderModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("OttiliCoder mode stack is not registered for this keymap")
  return value
}
...
export function registerOttiliCoderKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createOttiliCoderModeStack(keymap)
