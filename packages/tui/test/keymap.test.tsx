...
import { getOttiliCoderModeStack, OTTILI_CODER_BASE_MODE, OttiliCoderKeymapProvider, registerOttiliCoderKeymap } from "../src/keymap"
...
test("mode-less bindings stay active when ottili-coder mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getOttiliCoderModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getOttiliCoderModeStack(keymap).push("autocomplete")
