...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("ottili-coder-find")
  api.delete("ottili-coder-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("ottili-coder-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("ottili-coder-find", new Highlight(...rest))
    return true
  }
