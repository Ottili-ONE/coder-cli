...
There are two types of agents in Ottili Coder; primary agents and subagents.
...
Ottili Coder comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Ottili Coder comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

Ottili Coder comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into Ottili Coder's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `ottili-coder.json` config file:
...
- Per-project: `.ottili-coder/agents/`
...
If no temperature is specified, Ottili Coder uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Ottili Coder config and the project specific config.
...
The model ID in your Ottili Coder config uses the format `provider/model-id`. For example, if you're using [Ottili Coder](/docs/zen), you would use `ottili-coder/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `ottili-coder models` to see a list of the available models.
:::
