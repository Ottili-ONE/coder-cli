...
أنشئ الملف `.ottili-coder/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Ottili Coder أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Ottili Coder:
...
- على مستوى النظام: `~/.config/ottili-coder/commands/`
- لكل مشروع: `.ottili-coder/commands/`
...
يتضمن ottili-coder عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
