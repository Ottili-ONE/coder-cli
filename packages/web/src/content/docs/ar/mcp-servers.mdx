...
يمكنك إضافة أدوات خارجية إلى Ottili Coder باستخدام _Model Context Protocol_، أو MCP. يدعم Ottili Coder خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Ottili Coder](https://ottili.one/coder/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/ottili-coder` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Ottili Coder تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Ottili Coder بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Ottili Coder المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `ottili-coder mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Ottili Coder الرموز بشكل آمن في `~/.local/share/ottili-coder/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Ottili Coder إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Ottili Coder مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Ottili Coder بحساب Sentry الخاص بك.
