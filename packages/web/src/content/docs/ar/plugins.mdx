---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع Ottili Coder.
---

تتيح لك الإضافات توسيع Ottili Coder عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ Ottili Coder.
...
- `.ottili-coder/plugins/` - إضافات على مستوى المشروع
- `~/.config/ottili-coder/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/ottili-coder/node_modules/`.
...
1. إعدادات عامة (`~/.config/ottili-coder/ottili-coder.json`)
2. إعدادات المشروع (`ottili-coder.json`)
3. دليل الإضافات العام (`~/.config/ottili-coder/plugins/`)
4. دليل إضافات المشروع (`.ottili-coder/plugins/`)
...
يشغّل Ottili Coder الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ ottili-coder للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع ottili-coder.
...
:::note
إذا كنت تستخدم تطبيق Ottili Coder لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
امنع ottili-coder من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى ottili-coder:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ ottili-coder استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ ottili-coder إلى جانب الأدوات المضمنة.
...
المستويات: `debug`, `info`, `warn`, `error`. راجع [توثيق SDK](https://ottili.one/coder/docs/sdk) للتفاصيل.
