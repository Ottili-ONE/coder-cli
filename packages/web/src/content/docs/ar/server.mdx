---
title: الخادم
description: تفاعل مع خادم ottili-coder عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `ottili-coder serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل ottili-coder استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `ottili-coder.local` |
...
عيّن `OTTILI_CODER_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `ottili-coder`، أو عيّن `OTTILI_CODER_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `ottili-coder serve` و `ottili-coder web`.
...
عند تشغيل `ottili-coder` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم ottili-coder للتفاعل مع ottili-coder برمجيا.
:::

تتيح هذه البنية لـ ottili-coder دعم عدة عملاء وتمكّنك من التفاعل مع ottili-coder برمجيا.

يمكنك تشغيل `ottili-coder serve` لبدء خادم مستقل. إذا كانت واجهة ottili-coder في terminal (TUI)
قيد التشغيل، فسيبدأ `ottili-coder serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Ottili Coder لـ [IDE](/docs/ide).
...
يعرض خادم ottili-coder واجهات API التالية.
