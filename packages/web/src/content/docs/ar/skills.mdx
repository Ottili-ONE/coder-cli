...
تتيح مهارات الوكيل لـ Ottili Coder اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Ottili Coder في هذه المواقع:

- إعدادات المشروع: `.ottili-coder/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/ottili-coder/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Ottili Coder صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.ottili-coder/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/ottili-coder/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.ottili-coder/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Ottili Coder المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `ottili-coder.json`:
...
**للوكلاء المدمجين** (في `ottili-coder.json`):
