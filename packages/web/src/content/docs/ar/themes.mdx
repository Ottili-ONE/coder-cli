...
مع Ottili Coder يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Ottili Coder سمتنا `ottili-coder`.
...
يأتي Ottili Coder مع عدة سمات مدمجة.
...
- يريدون أن يطابق Ottili Coder مظهر terminal لديهم
...
يدعم Ottili Coder نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/ottili-coder/themes/*.json` أو `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.ottili-coder/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.ottili-coder/themes/*.json`
