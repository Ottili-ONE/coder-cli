---
title: TUI
description: استخدام واجهة المستخدم TUI في Ottili Coder.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يوفّر Ottili Coder واجهة terminal تفاعلية (TUI) للعمل على مشاريعك باستخدام LLM.

يؤدي تشغيل Ottili Coder إلى بدء واجهة TUI للدليل الحالي.
...
عند استخدام واجهة Ottili Coder في terminal، يمكنك كتابة `/` متبوعة باسم أمر لتنفيذ الإجراءات بسرعة. مثلا:
...
أضف موفّرا إلى Ottili Coder. يتيح لك اختيار أحد الموفّرين المتاحين وإضافة مفاتيح API الخاصة بهم.
...
اخرج من Ottili Coder. _الأسماء المستعارة_: `/quit`, `/q`
...
هذا منفصل عن `ottili-coder.json`، الذي يضبط سلوك الخادم ووقت التشغيل.
...
- `leader_timeout` - يتحكم في مدة انتظار Ottili Coder بعد الضغط على leader key. القيمة الافتراضية `2000`.
...
- `sound_pack` - sound pack ID المراد استخدامه. القيمة الافتراضية `ottili-coder.default`.
