...
Postoje dvije vrste agenata u Ottili Coder: primarni agenti i podagenti.
...
Ottili Coder dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Ottili Coder dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Ottili Coder dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u Ottili Coder-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `ottili-coder.json`:
...
- Globalno: `~/.config/ottili-coder/agents/`
- Po projektu: `.ottili-coder/agents/`
...
Ako temperatura nije navedena, Ottili Coder koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Ottili Coder konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Ottili Coder konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Ottili Coder](/docs/zen), koristili biste `ottili-coder/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `ottili-coder models` da vidite listu dostupnih modela.
:::
