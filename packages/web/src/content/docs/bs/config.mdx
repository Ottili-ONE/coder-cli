---
title: Konfiguracija
description: Korištenje Ottili Coder JSON konfiguracije.
---

Možete konfigurirati Ottili Coder koristeći JSON konfiguracijski fajl.

---

## Format

Ottili Coder podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/ottili-coder`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/ottili-coder/ottili-coder.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OTTILI_CODER_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`ottili-coder.json` u projektu) - postavke specifične za projekat
5. **`.ottili-coder` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`OTTILI_CODER_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.ottili-coder` i `~/.config/ottili-coder` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/ottili-coder` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Ottili Coder konfiguraciju u `~/.config/ottili-coder/ottili-coder.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/ottili-coder/tui.json`.
...
Dodajte `ottili-coder.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Ottili Coder pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `OTTILI_CODER_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OTTILI_CODER_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.ottili-coder` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.ottili-coder`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`ottili.one/coder/config.json`**](https://ottili.one/coder/config.json).

TUI konfiguracija koristi [**`ottili.one/coder/tui.json`**](https://ottili.one/coder/tui.json).
...
Koristite `OTTILI_CODER_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `ottili-coder.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `ottili-coder serve` i `ottili-coder web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Ottili Coder server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `ottili-coder.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Ottili Coder konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Ottili Coder pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/ottili-coder/agents/` ili `.ottili-coder/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Ottili Coder će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`ottili-coder run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/ottili-coder/commands/` ili `.ottili-coder/commands/`. [Saznajte više ovdje](/docs/commands).
...
Ottili Coder će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Ottili Coder **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Ottili Coder sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.ottili-coder/plugins/` ili `~/.config/ottili-coder/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Ottili Coder da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
