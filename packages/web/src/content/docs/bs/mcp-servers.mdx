...
Možete dodati vanjske alate u Ottili Coder koristeći _Model Context Protocol_, ili MCP. Ottili Coder podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Ottili Coder Config](https://ottili.one/coder/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/ottili-coder`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Ottili Coder automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Ottili Coder će:
...
Ako server zahtijeva autentifikaciju, Ottili Coder će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `ottili-coder mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Ottili Coder sigurno čuva tokene u `~/.local/share/ottili-coder/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Ottili Coder, zajedno s ugrađenim alatima. Možete njima upravljati kroz Ottili Coder konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Ottili Coder sa vašim Sentry nalogom.
