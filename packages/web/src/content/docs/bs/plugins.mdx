---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Ottili Coder.
---

Dodaci vam omogućavaju da proširite Ottili Coder spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Ottili Coder.
...
- `.ottili-coder/plugins/` - Dodaci na nivou projekta
- `~/.config/ottili-coder/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/ottili-coder/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/ottili-coder/ottili-coder.json`)
2. Konfiguracija projekta (`ottili-coder.json`)
3. Globalni direktorij dodataka (`~/.config/ottili-coder/plugins/`)
4. Direktorij dodataka projekta (`.ottili-coder/plugins/`)
...
Ottili Coder pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Ottili Coder.
...
Spriječite ottili-coder da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u ottili-coder:
...
Pomoćnik `tool` kreira prilagođeni alat koji ottili-coder može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za ottili-coder zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://ottili.one/coder/docs/sdk) za detalje.
