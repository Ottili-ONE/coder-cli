---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Ottili Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

Ottili Coder koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Ottili Coder konfiguraciji.
...
u `~/.local/share/ottili-coder/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Ottili Coder
config.
...
Ottili Coder je lista modela koje je obezbedio Ottili Coder tim koji su bili
testirano i potvrđeno da dobro radi sa Ottili Coder. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Ottili Coder.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite ottili-coder i idite na [ottili.one/coder/auth](https://ottili.one/coder/auth).
...
Radi kao i svaki drugi provajder u Ottili Coder i potpuno je opcionalan za korištenje.

---

## Ottili Coder Go

Ottili Coder Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Ottili Coder tim i koji su testirani i verificirani da dobro rade s Ottili Coder-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Ottili Coder Go` i idite na [ottili.one/coder/auth](https://ottili.one/coder/auth).
...
Da biste koristili Amazon Bedrock s Ottili Coder:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete ottili-coder:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `ottili-coder.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Ottili Coder.
:::
...
Možete konfigurirati ottili-coder za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi ottili-coder ispravno radio.
   :::
...
Također možete dodati modele kroz svoju ottili-coder konfiguraciju.
...
3.  Naziv: `Ottili Coder`, opseg: `api`
...
da pohrani token u ottili-coder auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Ottili Coder koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Ottili Coder
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`ottili-coder.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `ottili-coder.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s ottili-coder:
...
Za korištenje Google Vertex AI s Ottili Coder:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće ottili-coder.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki Ottili Coder razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Ottili Coder razgovor naveden kao zasebna sesija.
...
Možete konfigurirati ottili-coder za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati ottili-coder za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati ottili-coder za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Ottili Coder. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Ottili Coder:
...
3. Kopirajte API ključ za korištenje u Ottili Coder.
...
6. **Važno**: Prije upotrebe modela oblaka u Ottili Coder, morate lokalno povući informacije o modelu:
...
Ottili Coder je lista testiranih i verifikovanih modela koju je obezbedio Ottili Coder tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Ottili Coder</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Ottili Coder**.

```txt
   /connect
```

3. Unesite svoj Ottili Coder API ključ.
...
Također možete dodati dodatne modele putem vaše ottili-coder konfiguracije.
...
5. Također ih možete prilagoditi putem vaše ottili-coder konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju ottili-coder konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
4. Kreirajte ili ažurirajte svoju `ottili-coder.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Ottili Coder da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `ottili-coder auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Ottili Coder konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj ottili-coder konfiguraciji.
