---
title: Server
description: Komunicirajte s ottili-coder serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `ottili-coder serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji ottili-coder klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `ottili-coder.local` |
...
Postavite `OTTILI_CODER_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `ottili-coder`, ili postavite `OTTILI_CODER_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `ottili-coder serve` i za `ottili-coder web`.
...
Kada pokrenete `ottili-coder`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite ottili-coder server za programsku interakciju sa opencode-om.
:::

Ova arhitektura omogucava ottili-coder podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `ottili-coder serve` da startate standalone server. Ako je ottili-coder TUI vec pokrenut, `ottili-coder serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Ottili Coder [IDE](/docs/ide) pluginovi.
...
ottili-coder server izlaže sljedece API-je.
