...
Agent skills omogucavaju Ottili Coder da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Ottili Coder pretrazuje ove lokacije:

- Konfiguracija projekta: `.ottili-coder/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/ottili-coder/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Ottili Coder ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.ottili-coder/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/ottili-coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.ottili-coder/skills/git-release/SKILL.md` ovako:
...
Ottili Coder navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `ottili-coder.json`:
...
**Za ugradene agente** (u `ottili-coder.json`):
