...
U Ottili Coder mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Ottili Coder koristi nasu `ottili-coder` temu.
...
Ottili Coder dolazi sa vise ugradenih tema.
...
- Zele da Ottili Coder odgovara izgledu njihovog terminala
...
Ottili Coder podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/ottili-coder/themes/*.json` ili `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.ottili-coder/themes/*.json`
4. **Trenutni radni direktorij** - `./.ottili-coder/themes/*.json`
