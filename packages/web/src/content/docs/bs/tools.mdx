...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Ottili Coder dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Ottili Coder.
...
:::note
Ovaj alat je dostupan samo kada je `OTTILI_CODER_EXPERIMENTAL_LSP_TOOL=true` (ili `OTTILI_CODER_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Ottili Coder provajdera ili kada je varijabla `OTTILI_CODER_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Ottili Coder:
