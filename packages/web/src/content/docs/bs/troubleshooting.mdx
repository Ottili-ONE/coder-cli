...
Da biste otklonili probleme s Ottili Coder, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\ottili-coder\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `ottili-coder --log-level DEBUG`.

---

## Pohrana

Ottili Coder pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/ottili-coder/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\ottili-coder`
...
Ottili Coder Desktop pokreće lokalni Ottili Coder server (`ottili-coder-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Ottili Coder` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/ottili-coder/ottili-coder.jsonc` (ili `~/.config/ottili-coder/ottili-coder.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/ottili-coder/ottili-coder.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\ottili-coder\ottili-coder.jsonc`
...
Ottili Coder također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/ottili-coder/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\ottili-coder\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Ottili Coder mogao ponovo izgraditi.

1. Potpuno zatvorite Ottili Coder Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/ottili-coder`
- **Linux**: obrišite `~/.cache/ottili-coder` (ili pokrenite `rm -rf ~/.cache/ottili-coder`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\ottili-coder`

3. Ponovo pokrenite Ottili Coder Desktop.

---

### Rješavanje problema sa vezom na serveru

Ottili Coder Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `ottili-coder.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `OTTILI_CODER_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `OTTILI_CODER_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Ottili Coder Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Ottili Coder karakteristikama.

---

### Obavještenja se ne prikazuju

Ottili Coder Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Ottili Coder u postavkama vašeg OS-a, i
...
1. Zatvorite Ottili Coder Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Ottili Coder Desktop aplikacije):

- `ottili-coder.settings.dat` (URL zadanog servera za desktop)
- `ottili-coder.global.dat` i `ottili-coder.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s Ottili Coder:
...
Da saznate kojim modelima imate pristup, pokrenite `ottili-coder models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\ottili-coder`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Ottili Coder dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\ottili-coder`

2. Ponovo pokrenite Ottili Coder da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Ottili Coder da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Ottili Coder će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
