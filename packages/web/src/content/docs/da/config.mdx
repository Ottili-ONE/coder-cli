---
title: Konfiguration
description: Ved at bruge Ottili Coder JSON konfig.
---

Du kan konfigurere Ottili Coder ved hjælp af en JSON-konfigurationsfil.

---

## Format

Ottili Coder understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/ottili-coder`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/ottili-coder/ottili-coder.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`OTTILI_CODER_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`ottili-coder.json` i projekt) - projektspecifikke indstillinger
5. **`.ottili-coder` mapper** - agent, kommandoer, plugins
6. **Inline config** (`OTTILI_CODER_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.ottili-coder` og `~/.config/ottili-coder` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/ottili-coder`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale Ottili Coder-konfiguration i `~/.config/ottili-coder/ottili-coder.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `ottili-coder.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når Ottili Coder starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `OTTILI_CODER_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `OTTILI_CODER_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.ottili-coder` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.ottili-coder` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`ottili.one/coder/config.json`**](https://ottili.one/coder/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `ottili-coder serve` og `ottili-coder web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din Ottili Coder-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `ottili-coder.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din Ottili Coder-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger Ottili Coder at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din Ottili Coder-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/ottili-coder/agents/` eller `.ottili-coder/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil Ottili Coder falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`ottili-coder run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/ottili-coder/commands/` eller `.ottili-coder/commands/`. [Learn more here](/docs/commands).
...
Ottili Coder vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard ottili-coder **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide Ottili Coder med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.ottili-coder/plugins/` eller `~/.config/ottili-coder/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse Ottili Coder til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
