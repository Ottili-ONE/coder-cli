...
Du kan tilføje eksterne værktøjer til Ottili Coder ved hjælp af _Model Context Protocol_ eller MCP. Ottili Coder understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Ottili Coder Config](https://ottili.one/coder/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/ottili-coder`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
Ottili Coder håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Ottili Coder:
...
Hvis serveren kræver godkendelse, vil Ottili Coder bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `ottili-coder mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Ottili Coder opbevare tokens sikkert i `~/.local/share/ottili-coder/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem Ottili Coder-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Ottili Coder til din Sentry-konto.
