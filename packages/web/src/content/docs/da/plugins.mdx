---
title: Plugins
description: Skriv dine egne plugins for at udvide Ottili Coder.
---

Plugins giver dig mulighed for at udvide Ottili Coder ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Ottili Coders standardadfærd.
...
- `.ottili-coder/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/ottili-coder/node_modules/`.
...
1. Global konfiguration (`~/.config/ottili-coder/ottili-coder.json`)
2. Projektkonfiguration (`ottili-coder.json`)
3. Global plugin-mappe (`~/.config/ottili-coder/plugins/`)
4. Projekt plugin bibliotek (`.ottili-coder/plugins/`)
...
Ottili Coder kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En ottili-coder SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide ottili-coder.
...
:::note
Hvis du bruger Ottili Coder desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå ottili-coder i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til ottili-coder:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som ottili-coder kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for ottili-coder sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://ottili.one/coder/docs/sdk) for detaljer.
