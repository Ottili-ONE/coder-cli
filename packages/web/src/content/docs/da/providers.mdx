---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Ottili Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

Ottili Coder bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Ottili Coder-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/ottili-coder/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Ottili Coder-konfigurationen.
...
Ottili Coder er en liste over modeller leveret af Ottili Coder-teamet, som er blevet testet og verificeret til at fungere godt med Ottili Coder. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Ottili Coder.
:::

1. Kør kommandoen `/connect` i TUI, vælg ottili-coder og gå til [ottili.one/coder/auth](https://ottili.one/coder/auth).
...
Det fungerer som alle andre udbydere i Ottili Coder og er helt valgfrit at bruge.

---

## Ottili Coder Go

Ottili Coder Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Ottili Coder-teamet, som er testet og verificeret til at fungere godt med Ottili Coder.

1. Kør kommandoen `/connect` i TUI, vælg `Ottili Coder Go`, og gå til [ottili.one/coder/auth](https://ottili.one/coder/auth).
...
Sådan bruger du Amazon Bedrock med Ottili Coder:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører ottili-coder:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `ottili-coder.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Ottili Coder understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere ottili-coder til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at ottili-coder skal fungere korrekt.
   :::
...
gemme tokenet i ottili-coder auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Ottili Coder bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Ottili Coder
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`ottili-coder.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Tilpas gennem `ottili-coder.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Ottili Coder:
...
Sådan bruger du Google Vertex AI med Ottili Coder:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører ottili-coder.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem ottili-coder, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver Ottili Coder-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Ottili Coder-samtale opført som en separat session.
...
Du kan konfigurere ottili-coder til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere ottili-coder til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere ottili-coder til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Ottili Coder. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Ottili Coder:
...
3. Kopier API-nøglen til brug i Ottili Coder.
...
6. **Vigtigt**: Før du bruger skymodeller i Ottili Coder, skal du hente modelinformationen lokalt:
...
Ottili Coder er en liste over testede og verificerede modeller leveret af Ottili Coder-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Ottili Coder</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Ottili Coder**.

   ```txt
   /connect
   ```

3. Indtast Ottili Coder API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Ottili Coder:
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med ottili-coder. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `ottili-coder.json`-filen i projektkataloget:
...
`limit`-felterne lader Ottili Coder forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `ottili-coder auth list` for at se om legitimationsoplysningerne
