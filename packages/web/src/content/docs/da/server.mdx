...
Kommandoen `ottili-coder serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en opencode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `ottili-coder.local` |
...
Indstil `OTTILI_CODER_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `ottili-coder`, eller indstil `OTTILI_CODER_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `ottili-coder serve` og `ottili-coder web`.
...
Når du kører `ottili-coder` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug opencode-serveren til at interagere med ottili-coder programmatisk.
:::

Denne arkitektur lader ottili-coder understøtte flere klienter og lader dig interagere med ottili-coder programmatisk.

Du kan køre `ottili-coder serve` for at starte en selvstændig server. Hvis du har
ottili-coder TUI kørende, vil `ottili-coder serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Ottili Coder [IDE](/docs/ide) plugins.
...
Ottili Coder-serveren viser følgende API'er.
