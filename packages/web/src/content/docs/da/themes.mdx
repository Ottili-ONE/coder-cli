...
Med Ottili Coder kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Ottili Coder vårt eget `ottili-coder`-tema.
...
Ottili Coder kommer med flere innebygde temaer.
...
- Vil at Ottili Coder skal matche terminalens utseende
...
Ottili Coder støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/ottili-coder/themes/*.json` eller `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.ottili-coder/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.ottili-coder/themes/*.json`
