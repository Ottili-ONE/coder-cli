...
Es gibt zwei Arten von Agenten in Ottili Coder; Primäragenten und Subagenten.
...
Ottili Coder verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `ottili-coder.json`-Konfigurationsdatei:
...
- Pro Projekt: `.ottili-coder/agents/`
...
Wenn keine Temperatur angegeben ist, verwendet Ottili Coder modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Ottili Coder-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Ottili Coder-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Ottili Coder](/docs/zen) verwenden, würden Sie `ottili-coder/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `ottili-coder models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
