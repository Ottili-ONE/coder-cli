...
description: Erstellen Sie Tools, die der LLM in Ottili Coder aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von Ottili Coder wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.ottili-coder/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/ottili-coder/tools/` platzieren.
