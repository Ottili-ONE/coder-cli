...
Ottili Coder Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Ottili Coder. Du abonnierst Ottili Coder Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
Ottili Coder zu verwenden.
...
Ottili Coder Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Ottili Coder Go funktioniert wie jeder andere Provider in Ottili Coder.

1. Du meldest dich bei **<a href={console}>Ottili Coder</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Ottili Coder Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Ottili Coder Go abonnieren.
:::
...
Ottili Coder Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Ottili Coder Config verwendet das Format `opencode-go/<model-id>`. Für Kimi K2.6 würdest du beispielsweise `opencode-go/kimi-k2.6` in deiner Config verwenden.
...
Wir haben Ottili Coder Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Ottili Coder zu nutzen.
