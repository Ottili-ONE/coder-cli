...
Sie können externe Tools zu Ottili Coder hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Ottili Coder unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Ottili Coder Config](https://ottili.one/coder/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/ottili-coder`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Ottili Coder übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Ottili Coder Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Ottili Coder beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `ottili-coder mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Ottili Coder die Token sicher in `~/.local/share/ottili-coder/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Ottili Coder verfügbar. Sie können sie also wie jedes andere Tool über die Ottili Coder-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Ottili Coder mit Ihrem Sentry-Konto zu verbinden.
