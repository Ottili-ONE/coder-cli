---
title: Regeln
description: Lege eigene Anweisungen fuer ottili-coder fest.
---

Du kannst ottili-coder ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in ottili-coder den Befehl `/init` aus.
...
So kann ottili-coder sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht ottili-coder sie zu erweitern.
...
ottili-coder kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/ottili-coder/AGENTS.md` speichern.
...
Wenn du von Claude Code kommst, unterstuetzt Ottili Coder dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/ottili-coder/AGENTS.md` existiert)
...
Beim Start sucht ottili-coder in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/ottili-coder/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `ottili-coder.json` oder in `~/.config/ottili-coder/ottili-coder.json` angeben.
...
ottili-coder parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit ottili-coder.json

Empfohlen ist das `instructions`-Feld in der `ottili-coder.json`:
...
Du kannst ottili-coder explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `ottili-coder.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
