...
Agent Skills erlauben Ottili Coder, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Ottili Coder sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Ottili Coder vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.ottili-coder/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/ottili-coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.ottili-coder/skills/git-release/SKILL.md` zum Beispiel so:
...
Ottili Coder listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `ottili-coder.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `ottili-coder.json`):
