...
Mit Ottili Coder kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Ottili Coder das `ottili-coder`-Theme.
...
Ottili Coder liefert mehrere eingebaute Themes mit.
...
- Ottili Coder optisch ans Terminal anpassen wollen
...
Ottili Coder unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/ottili-coder/themes/*.json` oder `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.ottili-coder/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.ottili-coder/themes/*.json`
