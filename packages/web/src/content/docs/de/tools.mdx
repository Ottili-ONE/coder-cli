...
Ottili Coder bringt eingebaute Tools mit und laesst sich ueber [Custom Tools](/docs/custom-tools) oder [MCP-Server](/docs/mcp-servers) erweitern.
...
Hier sind alle in Ottili Coder verfuegbaren eingebauten Tools.
...
:::note
Dieses Tool ist nur verfuegbar, wenn `OTTILI_CODER_EXPERIMENTAL_LSP_TOOL=true` (oder `OTTILI_CODER_EXPERIMENTAL=true`) gesetzt ist.
:::
...
:::note
Dieses Tool ist nur verfuegbar, wenn der Ottili Coder-Provider genutzt wird oder die Umgebungsvariable `OTTILI_CODER_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.

Zum Aktivieren beim Start von Ottili Coder:
