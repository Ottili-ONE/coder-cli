...
Wenn Ottili Coder Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\ottili-coder\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `ottili-coder --log-level DEBUG`.

---

## Speicher

ottili-coder speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/ottili-coder/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\ottili-coder`
...
Ottili Coder Desktop startet im Hintergrund einen lokalen Ottili Coder-Server (`ottili-coder-cli`-Sidecar).
...
- Nur macOS: `Ottili Coder`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/ottili-coder/ottili-coder.jsonc` (oder `~/.config/ottili-coder/ottili-coder.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/ottili-coder/ottili-coder.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\ottili-coder\ottili-coder.jsonc`
...
Ottili Coder kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/ottili-coder/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\ottili-coder\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Ottili Coder ihn neu aufbauen kann.

1. Beende Ottili Coder Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/ottili-coder`
- **Linux**: loesche `~/.cache/ottili-coder` (oder fuehre aus: `rm -rf ~/.cache/ottili-coder`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\ottili-coder`

3. Starte Ottili Coder Desktop neu.

---

### Server-Verbindungsprobleme beheben

Ottili Coder Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `ottili-coder.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `OTTILI_CODER_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `OTTILI_CODER_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Ottili Coder Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Ottili Coder-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Ottili Coder Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Ottili Coder in den OS-Einstellungen aktiviert sind, und
...
1. Beende Ottili Coder Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Ottili Coder Desktop):

- `ottili-coder.settings.dat` (desktop default server URL)
- `ottili-coder.global.dat` und `ottili-coder.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Ottili Coder hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `ottili-coder models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\ottili-coder`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. ottili-coder installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\ottili-coder`

2. Starte ottili-coder neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt ottili-coder, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
ottili-coder erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
