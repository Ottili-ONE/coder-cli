...
Hay dos tipos de agentes en Ottili Coder; agentes primarios y subagentes.
...
Ottili Coder viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
Ottili Coder viene con tres subagentes integrados, **General**, **Explore** y **Scout**. Veremos esto a continuación.

---

## Integrados

Ottili Coder viene con dos agentes primarios integrados y tres subagentes integrados.
...
Un agente de solo lectura para investigar documentación externa y dependencias. Úsalo cuando necesites clonar el repositorio de una dependencia en la caché administrada de Ottili Coder, inspeccionar el código fuente de una librería o contrastar el código local con implementaciones upstream sin modificar tu espacio de trabajo.
...
Configure agentes en su archivo de configuración `ottili-coder.json`:
...
- Por proyecto: `.ottili-coder/agents/`
...
Si no se especifica ninguna temperatura, Ottili Coder utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global Ottili Coder como para la configuración específica del proyecto.
...
El ID del modelo en su configuración Ottili Coder usa el formato `provider/model-id`. Por ejemplo, si usa [Ottili Coder](/docs/zen), usaría `ottili-coder/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Ejecute `ottili-coder models` para ver una lista de los modelos disponibles.
:::
