...
description: Cree herramientas que LLM pueda llamar en ottili-coder.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de ottili-coder como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.ottili-coder/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/ottili-coder/tools/`.
...
```ts title=".ottili-coder/tools/database.ts" {1}
...
```ts title=".ottili-coder/tools/math.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".ottili-coder/tools/bash.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".ottili-coder/tools/project.ts" {8}
import { tool } from "@opencode-ai/plugin"
...
```python title=".ottili-coder/tools/add.py"
import sys
...
```ts title=".ottili-coder/tools/python-add.ts" {10}
import { tool } from "@opencode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".ottili-coder/tools/add.py")
