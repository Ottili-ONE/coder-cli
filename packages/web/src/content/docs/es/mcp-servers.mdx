...
Puede agregar herramientas externas a Ottili Coder utilizando el _Model Context Protocol_ o MCP. Ottili Coder admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Ottili Coder Config](https://ottili.one/coder/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/ottili-coder`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Ottili Coder maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Ottili Coder:
...
Si el servidor requiere autenticación, Ottili Coder le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `ottili-coder mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Ottili Coder almacenará los tokens de forma segura en `~/.local/share/ottili-coder/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Ottili Coder, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Ottili Coder como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Ottili Coder a su cuenta Sentry.
