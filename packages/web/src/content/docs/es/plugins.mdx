---
title: Complementos
description: Escriba sus propios complementos para extender Ottili Coder.
---

Los complementos le permiten extender Ottili Coder al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Ottili Coder.
...
- `.ottili-coder/plugins/` - Complementos a nivel de proyecto
- `~/.config/ottili-coder/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/ottili-coder/node_modules/`.
...
1. Configuración global (`~/.config/ottili-coder/ottili-coder.json`)
2. Configuración del proyecto (`ottili-coder.json`)
3. Directorio global de complementos (`~/.config/ottili-coder/plugins/`)
4. Directorio de complementos del proyecto (`.ottili-coder/plugins/`)
...
Ottili Coder ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK ottili-coder para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar ottili-coder.
...
:::note
Si está utilizando la aplicación de escritorio Ottili Coder, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que ottili-coder lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a ottili-coder:
...
El ayudante `tool` crea una herramienta personalizada a la que ottili-coder puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para ottili-coder junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://ottili.one/coder/docs/sdk) para obtener más detalles.
