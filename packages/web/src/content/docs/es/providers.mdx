---
title: Proveedores
description: Usando cualquier proveedor LLM en Ottili Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

Ottili Coder usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Ottili Coder.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Ottili Coder
configuración.
...
Ottili Coder es una lista de modelos proporcionados por el equipo Ottili Coder que han sido
probado y verificado para funcionar bien con Ottili Coder. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Ottili Coder.
:::

1. Ejecute el comando `/connect` en TUI, seleccione ottili-coder y diríjase a [ottili.one/coder/auth](https://ottili.one/coder/auth).
...
Funciona como cualquier otro proveedor en Ottili Coder y su uso es completamente opcional.

---

## Ottili Coder Go

Ottili Coder Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Ottili Coder que han sido
probado y verificado para funcionar bien con Ottili Coder.

1. Ejecute el comando `/connect` en TUI, seleccione `Ottili Coder Go` y diríjase a [ottili.one/coder/auth](https://ottili.one/coder/auth).
...
Para usar Amazon Bedrock con Ottili Coder:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta ottili-coder:
   ...
   Para una configuración persistente o específica del proyecto, utilice `ottili-coder.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Ottili Coder no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar ottili-coder para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que ottili-coder funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración ottili-coder.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `Ottili Coder`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación ottili-coder.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Ottili Coder utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Ottili Coder
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `ottili-coder.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `ottili-coder.json`:
...
Para utilizar su suscripción GitHub Copilot con ottili-coder:
...
Para utilizar Google Vertex AI con Ottili Coder:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta ottili-coder.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de ottili-coder, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación Ottili Coder como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Ottili Coder enumerada como una sesión separada.
...
Puede configurar ottili-coder para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar ottili-coder para usar modelos locales a través de LM Studio.
...
Puedes configurar ottili-coder para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Ottili Coder. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Ottili Coder:
...
3. Copie la clave API para usarla en Ottili Coder.
...
6. **Importante**: Antes de usar modelos en la nube en Ottili Coder, debe obtener la información del modelo localmente:
...
Ottili Coder es una lista de modelos probados y verificados proporcionada por el equipo Ottili Coder. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Ottili Coder</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Ottili Coder**.

   ```txt
   /connect
   ```

3. Ingrese su clave Ottili Coder API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración ottili-coder.
...
5. También puedes personalizarlos a través de tu configuración ottili-coder. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Ottili Coder:
...
También puede personalizar modelos a través de su configuración ottili-coder. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con ottili-coder. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `ottili-coder.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Ottili Coder comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `ottili-coder auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración ottili-coder y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración ottili-coder.
