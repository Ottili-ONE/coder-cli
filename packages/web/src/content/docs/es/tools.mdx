...
Las herramientas permiten que LLM realice acciones en su código base. Ottili Coder viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Ottili Coder.
...
```json title="ottili-coder.json" {4}
{
  "$schema": "https://ottili.one/coder/config.json",
  "permission": {
    "bash": "allow"
...
:::note
Esta herramienta solo está disponible cuando `OTTILI_CODER_EXPERIMENTAL_LSP_TOOL=true` (o `OTTILI_CODER_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Ottili Coder o cuando la variable de entorno `OTTILI_CODER_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Ottili Coder:

```bash
OTTILI_CODER_ENABLE_EXA=1 ottili-coder
```
