...
Para depurar problemas con Ottili Coder, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\ottili-coder\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `ottili-coder --log-level DEBUG`.

---

## Almacenamiento

ottili-coder almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/ottili-coder/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\ottili-coder`
...
Ottili Coder Desktop ejecuta un servidor Ottili Coder local (el sidecar `ottili-coder-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Ottili Coder` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/ottili-coder/ottili-coder.jsonc` (o `~/.config/ottili-coder/ottili-coder.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/ottili-coder/ottili-coder.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\ottili-coder\ottili-coder.jsonc`
...
Ottili Coder también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/ottili-coder/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\ottili-coder\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Ottili Coder pueda reconstruirlo.

1. Salga de Ottili Coder Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/ottili-coder`
- **Linux**: eliminar `~/.cache/ottili-coder` (o ejecutar `rm -rf ~/.cache/ottili-coder`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\ottili-coder`

3. Reinicie el escritorio Ottili Coder.

---

### Solucionar problemas de conexión del servidor

Ottili Coder Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `ottili-coder.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `OTTILI_CODER_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `OTTILI_CODER_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Ottili Coder requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Ottili Coder.

---

### Notificaciones que no se muestran

Ottili Coder Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Ottili Coder en la configuración de su sistema operativo, y
...
1. Salga del escritorio Ottili Coder.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Ottili Coder):

- `ottili-coder.settings.dat` (URL del servidor predeterminado de escritorio)
- `ottili-coder.global.dat` y `ottili-coder.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con Ottili Coder:
...
3. Asegúrese de tener la última versión con `ottili-coder upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `ottili-coder models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\ottili-coder`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. ottili-coder instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\ottili-coder`

2. Reinicie ottili-coder para reinstalar los paquetes del proveedor más recientes.

Esto obligará a ottili-coder a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
ottili-coder detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
