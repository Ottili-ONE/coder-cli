...
Il existe deux types d'agents dans Ottili Coder ; agents primaires et sous-agents.
...
Ottili Coder est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Ottili Coder est livré avec trois sous-agents intégrés, **General**, **Explore** et **Scout**. Nous les verrons ci-dessous.

---

## Agents intégrés

Ottili Coder est livré avec deux agents primaires intégrés et trois sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `ottili-coder.json` :
...
- Par projet : `.ottili-coder/agents/`
...
Si aucune température n'est spécifiée, Ottili Coder utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Ottili Coder et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Ottili Coder utilise le format `provider/model-id`. Par exemple, si vous utilisez [Ottili Coder](/docs/zen), vous utiliserez `ottili-coder/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `ottili-coder models` pour voir une liste des modèles disponibles.
:::
