---
title: Configuration
description: Utilisation de la configuration JSON Ottili Coder.
---

Vous pouvez configurer Ottili Coder à l'aide d'un fichier de configuration JSON.

---

## Format

Ottili Coder prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
1. **Configuration distante** (à partir de `.well-known/ottili-coder`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/ottili-coder/ottili-coder.json`) - préférences utilisateur
3. **Configuration personnalisée** (`OTTILI_CODER_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`ottili-coder.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.ottili-coder`** - agents, commandes, plugins
6. **Configuration en ligne** (`OTTILI_CODER_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.ottili-coder` et `~/.config/ottili-coder` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/ottili-coder`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
Placez votre configuration globale Ottili Coder dans `~/.config/ottili-coder/ottili-coder.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/ottili-coder/tui.json`.
...
Ajoutez `ottili-coder.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque Ottili Coder démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `OTTILI_CODER_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `OTTILI_CODER_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.ottili-coder`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.ottili-coder`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`ottili.one/coder/config.json`**](https://ottili.one/coder/config.json).

La configuration TUI utilise [**`ottili.one/coder/tui.json`**](https://ottili.one/coder/tui.json).
...
Utilisez `OTTILI_CODER_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `ottili-coder.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `ottili-coder serve` et `ottili-coder web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur Ottili Coder.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `ottili-coder.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration Ottili Coder via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, Ottili Coder essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/ottili-coder/agents/` ou `.ottili-coder/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, Ottili Coder reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`ottili-coder run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/ottili-coder/commands/` ou `.ottili-coder/commands/`. [En savoir plus ici](/docs/commands).
...
Ottili Coder téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, ottili-coder **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent Ottili Coder avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.ottili-coder/plugins/` ou `~/.config/ottili-coder/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
...
Ceci est utile lorsque vous souhaitez restreindre Ottili Coder à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
