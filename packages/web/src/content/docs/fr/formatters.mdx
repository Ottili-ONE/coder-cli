---
title: Formateurs
description: Ottili Coder utilise des formateurs spécifiques au langage.
---

Ottili Coder formate automatiquement les fichiers après leur écriture ou leur modification à l'aide de formateurs spécifiques au langage. Cela garantit que le code généré suit les styles de code de votre projet.

---

## Formateurs intégrés

Ottili Coder est livré avec plusieurs formateurs intégrés pour les langages et frameworks populaires. Vous trouverez ci-dessous une liste des formateurs, des extensions de fichiers prises en charge et des commandes ou options de configuration dont il a besoin.
...
Ainsi, si votre projet a `prettier` dans votre `package.json`, Ottili Coder l'utilisera automatiquement.

---

## Comment ça marche

Lorsque Ottili Coder écrit ou modifie un fichier, il :
...
Vous pouvez personnaliser les formateurs via la section `formatter` de votre configuration Ottili Coder.
...
```json title="ottili-coder.json" {5}
{
  "$schema": "https://ottili.one/coder/config.json",
  "formatter": {
    "prettier": {
...
```json title="ottili-coder.json" {4-14}
{
  "$schema": "https://ottili.one/coder/config.json",
  "formatter": {
    "prettier": {
