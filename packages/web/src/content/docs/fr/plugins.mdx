---
title: Plugins
description: Écrivez vos propres plugins pour étendre Ottili Coder.
---

Les plugins vous permettent d'étendre Ottili Coder en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Ottili Coder.
...
- `.ottili-coder/plugins/` - Plugins au niveau du projet
- `~/.config/ottili-coder/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/ottili-coder/node_modules/`.
...
1. Configuration globale (`~/.config/ottili-coder/ottili-coder.json`)
2. Configuration du projet (`ottili-coder.json`)
3. Répertoire global des plugins (`~/.config/ottili-coder/plugins/`)
4. Répertoire des plugins du projet (`.ottili-coder/plugins/`)
...
Ottili Coder exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK ottili-coder pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre ottili-coder.
...
:::note
Si vous utilisez l'application de bureau Ottili Coder, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez ottili-coder de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à ottili-coder :
...
L'assistant `tool` crée un outil personnalisé que ottili-coder peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour ottili-coder aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://ottili.one/coder/docs/sdk) pour plus de détails.
