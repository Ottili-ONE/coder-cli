---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Ottili Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

Ottili Coder utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Ottili Coder.
...
en `~/.local/share/ottili-coder/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Ottili Coder
configuration.
...
Ottili Coder est une liste de modèles fournis par l'équipe Ottili Coder qui ont été
testé et vérifié pour fonctionner correctement avec Ottili Coder. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Ottili Coder.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez ottili-coder et dirigez-vous vers [ottili.one/coder/auth](https://ottili.one/coder/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Ottili Coder et son utilisation est totalement facultative.

---

## Ottili Coder Go

Ottili Coder Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Ottili Coder qui ont été
testé et vérifié pour fonctionner correctement avec Ottili Coder.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `Ottili Coder Go` et rendez-vous sur [ottili.one/coder/auth](https://ottili.one/coder/auth).
...
Pour utiliser Amazon Bedrock avec Ottili Coder :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de ottili-coder :
...
Pour une configuration spécifique au projet ou persistante, utilisez `ottili-coder.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Ottili Coder n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer ottili-coder pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que ottili-coder fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration ottili-coder.
...
3.  Nom : `Ottili Coder`, Portées : `api`
...
pour stocker le jeton dans le stockage d'authentification ottili-coder.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Ottili Coder utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Ottili Coder
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `ottili-coder.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `ottili-coder.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec ottili-coder :
...
Pour utiliser Google Vertex AI avec Ottili Coder :
...
Définissez-les lors de l'exécution de ottili-coder.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via ottili-coder, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) pour enregistrer automatiquement chaque conversation Ottili Coder en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Ottili Coder répertoriée comme une session distincte.
...
Vous pouvez configurer ottili-coder pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer ottili-coder pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer ottili-coder pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Ottili Coder. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/opencode) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec Ottili Coder :
...
3. Copiez la clé API à utiliser dans Ottili Coder.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Ottili Coder, vous devez extraire les informations du modèle localement :
...
Ottili Coder est une liste de modèles testés et vérifiés fournie par l'équipe Ottili Coder. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Ottili Coder</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Ottili Coder**.

   ```txt
   /connect
   ```

3. Entrez votre clé Ottili Coder API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration ottili-coder.
...
5. Vous pouvez également les personnaliser via votre configuration ottili-coder. Voici un exemple de spécification d'un fournisseur
...
Vous pouvez également personnaliser les modèles via votre configuration ottili-coder. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec ottili-coder. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `ottili-coder.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à Ottili Coder de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `ottili-coder auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration ottili-coder et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration ottili-coder.
