---
title: SDK
description: Client JS de type sécurisé pour le serveur ottili-coder.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK ottili-coder JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler ottili-coder par programme.
...
Créez une instance de ottili-coder :
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `ottili-coder.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createOpencode } from "@opencode-ai/sdk"

const ottili-coder = await createOpencode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${ottili-coder.server.url}`)

ottili-coder.server.close()
```

## Client uniquement

Si vous disposez déjà d'une instance en cours d'exécution de ottili-coder, vous pouvez créer une instance client pour vous y connecter :
...
const textResults = await client.find.text({
  query: { pattern: "function.*ottili-coder" },
})
