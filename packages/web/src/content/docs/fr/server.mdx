---
title: Serveur
description: Interagissez avec le serveur ottili-coder via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `ottili-coder serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client ottili-coder peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `ottili-coder.local` |
...
Définissez `OTTILI_CODER_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `ottili-coder`, ou définissez `OTTILI_CODER_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `ottili-coder serve` et à `ottili-coder web`.
...
Lorsque vous exécutez `ottili-coder`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur ottili-coder pour interagir avec ottili-coder par programmation.
:::

Cette architecture permet à ottili-coder de prendre en charge plusieurs clients et vous permet d'interagir avec ottili-coder par programme.

Vous pouvez exécuter `ottili-coder serve` pour démarrer un serveur autonome. Si vous avez le
ottili-coder TUI en cours d'exécution, `ottili-coder serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Ottili Coder [IDE](/docs/ide).
...
Le serveur ottili-coder expose les API suivantes.
