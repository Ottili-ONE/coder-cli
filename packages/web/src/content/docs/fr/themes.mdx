...
Avec Ottili Coder, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Ottili Coder utilise notre propre thème `ottili-coder`.
...
Ottili Coder est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Ottili Coder corresponde à l'apparence de leur terminal
...
Ottili Coder prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/ottili-coder/themes/*.json` ou `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.ottili-coder/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.ottili-coder/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/ottili-coder/themes
vim ~/.config/ottili-coder/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .ottili-coder/themes
vim .ottili-coder/themes/my-theme.json
```
