---
title: TUI
description: Utilisation de l’interface utilisateur du terminal Ottili Coder.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Ottili Coder fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de Ottili Coder démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le Ottili Coder TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à Ottili Coder. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Quittez Ottili Coder. *Alias* : `/quit`, `/q`
...
Ceci est séparé de `ottili-coder.json`, qui configure le comportement du serveur/d'exécution.
...
- `leader_timeout` - Contrôle la durée pendant laquelle Ottili Coder attend après la leader key. Valeur par défaut : `2000`.
...
- `sound_pack` - ID du sound pack à utiliser. Valeur par défaut : `ottili-coder.default`.
