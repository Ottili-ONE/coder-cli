...
In Ottili Coder esistono due tipi di agenti: agenti primari e subagenti.
...
Ottili Coder include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Ottili Coder include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

Ottili Coder include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di Ottili Coder, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Configura gli agenti nel file `ottili-coder.json`:
...
- Globale: `~/.config/ottili-coder/agents/`
- Per progetto: `.ottili-coder/agents/`
...
Se non specifichi una temperature, Ottili Coder usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Ottili Coder sia per la config specifica del progetto.
...
L'ID modello nella configurazione Ottili Coder usa il formato `provider/model-id`. Per esempio, se stai usando [Ottili Coder](/docs/zen), useresti `ottili-coder/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `ottili-coder models` per vedere la lista dei modelli disponibili.
:::
