---
title: Server LSP
description: Ottili Coder si integra con i tuoi server LSP.
---

Ottili Coder può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

Ottili Coder include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `OTTILI_CODER_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e ottili-coder apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di ottili-coder.
