...
Puoi aggiungere strumenti esterni a Ottili Coder usando il _Model Context Protocol_, o MCP. Ottili Coder supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Ottili Coder Config](https://ottili.one/coder/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/ottili-coder`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Ottili Coder gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Ottili Coder:
...
Se il server richiede autenticazione, Ottili Coder ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `ottili-coder mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Ottili Coder memorizzera' i token in modo sicuro in `~/.local/share/ottili-coder/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Ottili Coder insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Ottili Coder come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Ottili Coder al tuo account Sentry.
