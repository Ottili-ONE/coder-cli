---
title: Plugin
description: Scrivi plugin per estendere Ottili Coder.
---

I plugin ti permettono di estendere Ottili Coder agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Ottili Coder.
...
- `.ottili-coder/plugins/` - plugin a livello progetto
- `~/.config/ottili-coder/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/ottili-coder/node_modules/`.
...
1. Config globale (`~/.config/ottili-coder/ottili-coder.json`)
2. Config di progetto (`ottili-coder.json`)
3. Directory plugin globale (`~/.config/ottili-coder/plugins/`)
4. Directory plugin di progetto (`.ottili-coder/plugins/`)
...
Ottili Coder esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di ottili-coder per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere ottili-coder.
...
:::note
Se usi l'app desktop di Ottili Coder, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a ottili-coder di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a ottili-coder:
...
L'helper `tool` crea uno strumento personalizzato che ottili-coder può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in ottili-coder insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://ottili.one/coder/docs/sdk) per i dettagli.
