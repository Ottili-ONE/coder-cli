---
title: Provider
description: Usa qualsiasi provider LLM in Ottili Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

Ottili Coder usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Ottili Coder.
...
configurazione di Ottili Coder.
...
Ottili Coder e una lista di modelli forniti dal team Ottili Coder, testati e
verificati per funzionare bene con Ottili Coder. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Ottili Coder.
:::

1. Esegui `/connect` nella TUI, seleziona ottili-coder e vai su [ottili.one/coder/auth](https://ottili.one/coder/auth).
...
Funziona come qualsiasi altro provider in Ottili Coder ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Ottili Coder:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui ottili-coder:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `ottili-coder.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Ottili Coder non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare ottili-coder per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché ottili-coder funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di ottili-coder.
...
2. Scegli il tuo metodo di autenticazione:
   ...
   3. Nome: `Ottili Coder`, Scopes: `api`
...
salvare il token nello storage di auth di ottili-coder.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Ottili Coder usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Ottili Coder
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `ottili-coder.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Personalizza tramite `ottili-coder.json`:
...
Per usare il tuo abbonamento GitHub Copilot con ottili-coder:
...
Per usare Google Vertex AI con Ottili Coder:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui ottili-coder.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da ottili-coder, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di Ottili Coder come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Ottili Coder elencata come sessione separata.
...
Puoi configurare ottili-coder per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare ottili-coder per usare modelli locali tramite LM Studio.
...
Puoi configurare ottili-coder per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per Ottili Coder. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con Ottili Coder:
...
3. Copia la chiave API da usare in Ottili Coder.
...
6. **Importante**: Prima di usare modelli cloud in Ottili Coder, devi scaricare le informazioni del modello localmente:
...
Ottili Coder è una lista di modelli testati e verificati forniti dal team Ottili Coder. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Ottili Coder</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Ottili Coder**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Ottili Coder.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di ottili-coder.
...
5. Puoi anche personalizzarli tramite la configurazione di ottili-coder. Ecco un esempio di specifica di un provider
...
Puoi anche personalizzare i modelli tramite la tua configurazione di ottili-coder. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con ottili-coder. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `ottili-coder.json` nella directory del progetto:
...
I campi `limit` permettono a Ottili Coder di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `ottili-coder auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di ottili-coder e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config ottili-coder.
