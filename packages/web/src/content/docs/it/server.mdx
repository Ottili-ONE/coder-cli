---
title: Server
description: Interagisci con il server ottili-coder via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `ottili-coder serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client ottili-coder.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `ottili-coder.local` |
...
Imposta `OTTILI_CODER_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `ottili-coder`, oppure imposta `OTTILI_CODER_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `ottili-coder serve` sia per `ottili-coder web`.
...
Quando esegui `ottili-coder` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server ottili-coder per interagire con ottili-coder in modo programmatico.
:::

Questa architettura permette a ottili-coder di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `ottili-coder serve` per avviare un server standalone. Se la TUI di ottili-coder e gia in esecuzione, `ottili-coder serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Ottili Coder.
...
Il server ottili-coder espone le seguenti API.
