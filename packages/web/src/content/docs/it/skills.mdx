...
Le skill degli agenti permettono a Ottili Coder di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Ottili Coder cerca in queste posizioni:

- Config di progetto: `.ottili-coder/skills/<name>/SKILL.md`
- Config globale: `~/.config/ottili-coder/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Ottili Coder risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.ottili-coder/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/ottili-coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.ottili-coder/skills/git-release/SKILL.md` cosi':
...
Ottili Coder elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `ottili-coder.json`:
...
**Per agenti integrati** (in `ottili-coder.json`):
