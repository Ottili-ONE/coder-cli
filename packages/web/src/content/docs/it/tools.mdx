...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. Ottili Coder include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in Ottili Coder.
...
:::note
Questo strumento e' disponibile solo quando `OTTILI_CODER_EXPERIMENTAL_LSP_TOOL=true` (oppure `OTTILI_CODER_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider Ottili Coder o quando la variabile d'ambiente `OTTILI_CODER_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di Ottili Coder:
