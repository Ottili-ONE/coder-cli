---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Ottili Coder.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Ottili Coder fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Ottili Coder avvia la TUI per la directory corrente.
...
Quando usi la TUI di Ottili Coder, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Ottili Coder. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Ottili Coder. _Alias_: `/quit`, `/q`
...
Questo è separato da `ottili-coder.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Ottili Coder attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `ottili-coder.default`.
