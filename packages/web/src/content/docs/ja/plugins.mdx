---
title: プラグイン
description: Ottili Coder を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Ottili Coder を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Ottili Coder のデフォルトの動作を変更したりできます。
...
- `.ottili-coder/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/ottili-coder/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/ottili-coder/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/ottili-coder/ottili-coder.json`)
2. プロジェクト設定 (`ottili-coder.json`)
3. グローバルプラグインディレクトリ (`~/.config/ottili-coder/plugins/`)
4. プロジェクトプラグインディレクトリ (`.ottili-coder/plugins/`)
...
Ottili Coder は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Ottili Coder SDK クライアント。
...
ここでは、Ottili Coder を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://ottili.one/coder/docs/sdk) を参照してください。
...
Ottili Coder が `.env` ファイルを読み取らないようにします。
...
プラグインは Ottili Coder にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Ottili Coder が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Ottili Coder で使用できます。
