...
Agent Skill let ottili-coder discover reusable instruction from your repo 또는 홈 디렉토리.
...
ottili-coder 이 위치를 검색:

- 프로젝트 구성: `.ottili-coder/skills/<name>/SKILL.md`
- 글로벌 구성: `~/.config/ottili-coder/skills/<name>/SKILL.md`
...
그것은 `skills/*/SKILL.md`에 있는 어떤 어울리는 `.ottili-coder/` 및 어떤 어울리는 `.claude/skills/*/SKILL.md` 또는 `.agents/skills/*/SKILL.md`를 방법 적재합니다.

세계적인 정의는 또한 `~/.config/ottili-coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` 및 `~/.agents/skills/*/SKILL.md`에서 적재됩니다.
...
이처럼 `.ottili-coder/skills/git-release/SKILL.md` 만들기:
...
기술 에이전트가 `ottili-coder.json`의 패턴 기반 권한을 사용하여 액세스 할 수있는 제어 :
