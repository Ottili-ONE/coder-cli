...
-**macOS/리눅스**: `~/.local/share/ottili-coder/log/`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.local\share\ottili-coder\log`

로그 파일은 타임스탬프 (예 : `2025-01-09T123456.log`)과 가장 최근 10 로그 파일이 보관됩니다.

자세한 디버그 정보를 얻기 위해 `--log-level` 명령줄 옵션을 사용하여 로그 레벨을 설정할 수 있습니다. 예를 들면, `ottili-coder --log-level DEBUG`.

---

## 저장소

ottili-coder 저장 세션 데이터 및 디스크에 다른 응용 데이터:

-**macOS/리눅스**: `~/.local/share/ottili-coder/`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.local\share\ottili-coder`
...
ottili-coder 데스크톱은 배경에서 로컬 ottili-coder 서버 (`ottili-coder-cli` sidecar)를 실행합니다. 대부분의 문제는 misbehaving 플러그인, 손상된 캐시, 또는 나쁜 서버 설정에 의해 발생합니다.

## 빠른 확인

- 완전히 종료하고 앱을 다시 시작.
- 앱이 오류 화면을 보여 주면 ** Restart**를 클릭하고 오류 세부 정보를 복사합니다.
- macOS만: `Ottili Coder` 메뉴 -> **웹뷰 **(UI가 공백/frozen인 경우).
...
-**macOS/리눅스**: `~/.config/ottili-coder/ottili-coder.jsonc` (또는 `~/.config/ottili-coder/ottili-coder.json`) -**macOS/Linux** (외부 설치): `~/.local/share/ottili-coder/ottili-coder.jsonc`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.config\ottili-coder\ottili-coder.jsonc`
...
- **글로벌 플러그인** -**macOS/리눅스**: `~/.config/ottili-coder/plugins/`
- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.config\ottili-coder\plugins`
...
1. Quit ottili-coder 데스크톱 완전히.
2. 캐시 디렉토리 삭제:

-**macOS**: 찾기 -> `Cmd+Shift+G` -> 붙여넣기 `~/.cache/ottili-coder`

- **Linux**: `~/.cache/ottili-coder` 삭제 (또는 `rm -rf ~/.cache/ottili-coder` 실행)
- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.cache\ottili-coder`

3. Restart ottili-coder 데스크탑.

---

## 서버 연결 문제 수정

ottili-coder 데스크톱은 자체 로컬 서버(기본)를 시작하거나 구성된 서버 URL에 연결할 수 있습니다.
...
`ottili-coder.json(c)`가 `server` 섹션을 포함하면 일시적으로 제거하고 데스크톱 앱을 다시 시작합니다.

### 환경 변수 확인

`OTTILI_CODER_PORT`가 있는 경우, 데스크탑 앱은 로컬 서버의 포트를 사용하려고 합니다.

- `OTTILI_CODER_PORT` (또는 무료 포트를 선택) 및 재시작.
...
Windows에서 ottili-coder 데스크톱은 Microsoft Edge ** WebView2 실행 시간 **를 요구합니다. 앱이 공백 창에 열거나 시작하지 않을 경우, install/update WebView2를 설치하고 다시 시도하십시오.
...
ottili-coder 데스크톱은 시스템 알림을 보여줍니다 :
...
1. Quit ottili-coder 데스크탑.
2. 이 파일을 찾아 삭제 (ottili-coder 데스크톱 앱 데이터 디렉토리에서 라이브):

- `ottili-coder.settings.dat` (데스크톱 기본 서버 URL)
- `ottili-coder.global.dat` 및 `ottili-coder.workspace.*.dat` (최근 서버/프로젝트와 같은 UI 국가)
...
3. 당신은 `ottili-coder upgrade`를 가진 최신 버전이 있는 것을 지킵니다
...
당신이 액세스 할 수있는 모델을 파악하려면, `ottili-coder models`를 실행
...
Windows에서, `WIN+R`를 누르고 삭제하십시오: `%USERPROFILE%\.local\share\ottili-coder`
...
API 호출 오류가 발생하면, 이 공급 업체 패키지로 인해 발생할 수 있습니다. ottili-coder 동적 설치 공급자 패키지 (OpenAI, Anthropic, Google 등) 필요 하 고 로컬로 캐시.
...
Windows에서, `WIN+R`를 누르고 삭제하십시오: `%USERPROFILE%\.cache\ottili-coder`

2. 최신 공급자 포장을 재설치하는 Restart ottili-coder
