---
title: LSP Servers
description: Ottili Coder integrates with your LSP servers.
---

Ottili Coder can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

Ottili Coder comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `OTTILI_CODER_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and ottili-coder opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your ottili-coder config.
...
```json title="ottili-coder.json" {5-8}
{
  "$schema": "https://ottili.one/coder/config.json",
  "lsp": {
    "rust": {
...
```json title="ottili-coder.json" {5-13}
{
  "$schema": "https://ottili.one/coder/config.json",
  "lsp": {
    "custom-lsp": {
