...
Det er to typer agenter i Ottili Coder; primære agenter og underagenter.
...
Ottili Coder kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Ottili Coder kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Ottili Coder kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `ottili-coder.json`:
...
- Per prosjekt: `.ottili-coder/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Ottili Coder modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Ottili Coder-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Ottili Coder-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Ottili Coder](/docs/zen), vil du bruke `ottili-coder/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `ottili-coder models` for å se en liste over tilgjengelige modeller.
:::
