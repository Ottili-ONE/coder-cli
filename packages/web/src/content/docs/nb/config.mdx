---
title: Konfigurasjon
description: Ved å bruke ottili-coder JSON-konfigurasjonen.
---

Du kan konfigurere Ottili Coder ved å bruke en JSON konfigurasjonsfil.

---

## Format

Ottili Coder støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "ottili-coder"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/ottili-coder`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/ottili-coder/ottili-coder.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`OTTILI_CODER_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`ottili-coder.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.ottili-coder` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`OTTILI_CODER_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.ottili-coder` og `~/.config/ottili-coder` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/ottili-coder`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale Ottili Coder-konfigurasjon i `~/.config/ottili-coder/ottili-coder.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `ottili-coder.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når Ottili Coder starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `OTTILI_CODER_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `OTTILI_CODER_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.ottili-coder` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.ottili-coder`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`ottili.one/coder/config.json`**](https://ottili.one/coder/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `ottili-coder serve` og `ottili-coder web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage Ottili Coder-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `ottili-coder.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i Ottili Coder-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Ottili Coder å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/ottili-coder/agents/` eller `.ottili-coder/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Ottili Coder falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`ottili-coder run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/ottili-coder/commands/` eller `.ottili-coder/commands/`. [Les mer her](/docs/commands).
...
Ottili Coder vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Ottili Coder alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Ottili Coder med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.ottili-coder/plugins/` eller `~/.config/ottili-coder/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense Ottili Coder til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
