...
Du kan legge til eksterne verktøy i Ottili Coder ved å bruke _Model Context Protocol_ eller MCP. Ottili Coder støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Ottili Coder Config](https://ottili.one/coder/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="ottili-coder.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/ottili-coder`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="ottili-coder.jsonc" {15}
{
  "$schema": "https://ottili.one/coder/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
Ottili Coder håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Ottili Coder:
...
Hvis serveren krever autentisering, vil Ottili Coder be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `ottili-coder mcp auth <server-name>`.
...
```json title="ottili-coder.json" {7-11}
{
  "$schema": "https://ottili.one/coder/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
ottili-coder mcp auth my-oauth-server
```
...
```bash
ottili-coder mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Ottili Coder lagre tokens sikkert i `~/.local/share/ottili-coder/mcp-auth.json`.
...
```json title="ottili-coder.json" {7}
{
  "$schema": "https://ottili.one/coder/config.json",
  "mcp": {
    "my-api-key-server": {
...
ottili-coder mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Ottili Coder, sammen med innebygde verktøy. Så du kan administrere dem gjennom Ottili Coder-konfigurasjonen som alle andre verktøy.
...
```json title="ottili-coder.json" {14}
{
  "$schema": "https://ottili.one/coder/config.json",
  "mcp": {
    "my-mcp-foo": {
...
```json title="ottili-coder.json" {11, 14-18}
{
  "$schema": "https://ottili.one/coder/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="ottili-coder.json" {4-8}
{
  "$schema": "https://ottili.one/coder/config.json",
  "mcp": {
    "sentry": {
...
```bash
ottili-coder mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Ottili Coder til Sentry-kontoen din.
...
```json title="ottili-coder.json" {4-7}
{
  "$schema": "https://ottili.one/coder/config.json",
  "mcp": {
    "context7": {
...
```json title="ottili-coder.json" {7-9}
{
  "$schema": "https://ottili.one/coder/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Ottili Coder-samtale oppført som en separat økt.
