...
description: Skriv dine egne programtillegg for å utvide Ottili Coder.
---

Plugins lar deg utvide Ottili Coder ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Ottili Coder.
...
- `.ottili-coder/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/ottili-coder/node_modules/`.
...
1. Global konfigurasjon (`~/.config/ottili-coder/ottili-coder.json`)
2. Prosjektkonfigurasjon (`ottili-coder.json`)
3. Global plugin-katalog (`~/.config/ottili-coder/plugins/`)
4. Prosjektpluginkatalog (`.ottili-coder/plugins/`)
...
Ottili Coder kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Ottili Coder SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Ottili Coder.
...
:::note
Hvis du bruker Ottili Coder-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Ottili Coder fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Ottili Coder:
...
`tool`-hjelperen lager et tilpasset verktøy som Ottili Coder kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Ottili Coder sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://ottili.one/coder/docs/sdk) for detaljer.
