---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Ottili Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

Ottili Coder bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Ottili Coder-konfigurasjonen.
...
i `~/.local/share/ottili-coder/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Ottili Coder
konfig.
...
Ottili Coder er en liste over modeller levert av Ottili Coder-teamet som har vært
testet og verifisert for å fungere godt med Ottili Coder. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Ottili Coder.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Ottili Coder` og gå til [ottili.one/coder/auth](https://ottili.one/coder/auth).
...
Det fungerer som alle andre leverandører i Ottili Coder og er helt valgfritt å bruke.

---

## Ottili Coder Go

Ottili Coder Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Ottili Coder-teamet som har vært
testet og verifisert for å fungere godt med Ottili Coder.

1. Kjør kommandoen `/connect` i TUI, velg `Ottili Coder Go`, og gå til [ottili.one/coder/auth](https://ottili.one/coder/auth).
...
Slik bruker du Amazon Bedrock med Ottili Coder:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Ottili Coder:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `ottili-coder.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i ottili-coder støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere ottili-coder til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Ottili Coder skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Ottili Coder-konfigurasjonen.
...
2. Velg autentiseringsmetoden din:
   ...
   3. Navn: `Ottili Coder`, omfang: `api`
...
for å lagre token i Ottili Coder auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Ottili Coder bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Ottili Coder
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`ottili-coder.json` fil. Det anbefales også å deaktivere øktdeling.
...
Tilpass gjennom `ottili-coder.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Ottili Coder:
...
Slik bruker du Google Vertex AI med Ottili Coder:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Ottili Coder.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Ottili Coder, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver Ottili Coder-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Ottili Coder-samtale oppført som en separat økt.
...
Du kan konfigurere Ottili Coder for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Ottili Coder for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Ottili Coder for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Ottili Coder. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Ottili Coder:
...
3. Kopier API-nøkkelen for bruk i Ottili Coder.
...
6. **Viktig**: Før du bruker skymodeller i Ottili Coder, må du hente modellinformasjonen lokalt:
...
Ottili Coder er en liste over testede og verifiserte modeller levert av Ottili Coder-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Ottili Coder</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Ottili Coder**.

   ```txt
   /connect
   ```

3. Skriv inn Ottili Coder API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Ottili Coder-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Ottili Coder-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Ottili Coder:
...
Du kan også tilpasse modeller gjennom Ottili Coder-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Ottili Coder. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `ottili-coder.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Ottili Coder forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `ottili-coder auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Ottili Coder-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Ottili Coder-konfigurasjonen.
