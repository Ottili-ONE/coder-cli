---
title: Tjener
description: Samhandle med Ottili Coder-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `ottili-coder serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Ottili Coder-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `ottili-coder.local` |
...
Sett `OTTILI_CODER_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `ottili-coder` som standard, eller sett `OTTILI_CODER_SERVER_USERNAME` for å overstyre det. Dette gjelder både `ottili-coder serve` og `ottili-coder web`.
...
Når du kjører `ottili-coder` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Ottili Coder-serveren til å samhandle med Ottili Coder programmatisk.
:::

Denne arkitekturen lar Ottili Coder støtte flere klienter og lar deg samhandle med Ottili Coder programmatisk.

Du kan kjøre `ottili-coder serve` for å starte en frittstående server. Hvis du har
Ottili Coder TUI kjører, vil `ottili-coder serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Ottili Coder [IDE](/docs/ide) plugins.
