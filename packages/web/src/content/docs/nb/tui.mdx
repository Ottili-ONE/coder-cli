...
Ottili Coder gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre Ottili Coder starter TUI for gjeldende katalog.
...
Når du bruker Ottili Coder TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til Ottili Coder. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt Ottili Coder. _Aliaser_: `/quit`, `/q`
...
Dette er atskilt fra `ottili-coder.json`, som konfigurerer server-/kjøretidsoppførsel.
...
- `leader_timeout` - Styrer hvor lenge Ottili Coder venter etter leader key. Standard er `2000`.
...
- `sound_pack` - Sound pack-ID som skal brukes. Standard er `ottili-coder.default`.
