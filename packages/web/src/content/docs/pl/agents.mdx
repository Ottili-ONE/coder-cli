...
W Ottili Coder obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Ottili Coder zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Ottili Coder ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

Ottili Coder ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej Ottili Coder, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
Skonfiguruj agenta w pliku `ottili-coder.json`:
...
- Globalnie: `~/.config/ottili-coder/agents/`
- W projekcie: `.ottili-coder/agents/`
...
Jeśli nie zostanie skonfigurowana, Ottili Coder używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Ottili Coder, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Ottili Coder w `provider/model-id`. Na przykład, [Ottili Coder](/docs/zen), użyj `ottili-coder/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `ottili-coder models`, aby zobaczyć dostępne modele.
:::
