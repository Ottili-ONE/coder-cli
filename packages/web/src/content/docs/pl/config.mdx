---
title: Konfiguracja
description: Ustawienia Ottili Coder w pliku JSON.
---

Możesz dostosować Ottili Coder za pomocą pliku konfiguracyjnego JSON.

---

## Format

Ottili Coder obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "ottili-coder"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/ottili-coder`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/ottili-coder/ottili-coder.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `OTTILI_CODER_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`ottili-coder.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.ottili-coder`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `OTTILI_CODER_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.ottili-coder` i `~/.config/ottili-coder` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/ottili-coder`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację Ottili Coder w `~/.config/ottili-coder/ottili-coder.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/ottili-coder/tui.json`.
...
Dodaj `ottili-coder.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy Ottili Coder się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `OTTILI_CODER_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `OTTILI_CODER_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.ottili-coder` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.ottili-coder`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`ottili.one/coder/config.json`**](https://ottili.one/coder/config.json).
...
Użyj `OTTILI_CODER_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `ottili-coder.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `ottili-coder serve` i `ottili-coder web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer Ottili Coder.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `ottili-coder.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie Ottili Coder wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/ottili-coder/agents/` lub `.ottili-coder/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, Ottili Coder powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`ottili-coder run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/ottili-coder/commands/` lub `.ottili-coder/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Ottili Coder może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy Ottili Coder nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie Ottili Coder **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają Ottili Coder o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.ottili-coder/plugins/` lub `~/.config/ottili-coder/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie Ottili Coder tylko do określonych dostawców.
