...
Możesz dodać dodatkowe informacje do ottili-coder za pomocą _Model Context Protocol_, w skrócie MCP. ottili-coder obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [ottili-coder Config](https://ottili.one/coder/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/ottili-coder`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
ottili-coder automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, ottili-coder:
...
Jeśli serwer wymaga uwierzytelnienia, ottili-coder poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `ottili-coder mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji ottili-coder bezpieczne przechowa tokeny w `~/.local/share/ottili-coder/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w ottili-coder, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie ottili-coder, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć ottili-coder z kontem Sentry.
