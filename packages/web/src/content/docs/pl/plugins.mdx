---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć ottili-coder.
---

Wtyczki umożliwiające wyłączenie ottili-coder poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania ottili-coder.
...
- `.ottili-coder/plugins/` - Wtyczki na poziomie projektu
- `~/.config/ottili-coder/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/ottili-coder/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/ottili-coder/ottili-coder.json`)
2. Konfiguracja projektu (`ottili-coder.json`)
3. Globalny katalog wtyczek (`~/.config/ottili-coder/plugins/`)
4. Katalog wtyczek projektu (`.ottili-coder/plugins/`)
...
ottili-coder uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z ottili-coder do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do ottili-coder.
...
:::note
Jeśli korzystasz z aplikacji komputerowej ottili-coder, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj ottili-coder czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do ottili-coder:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod ottili-coder. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://ottili.one/coder/docs/sdk).
