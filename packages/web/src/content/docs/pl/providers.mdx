---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w ottili-coder.
---

import config from "../../../../config.mjs"
export const console = config.console

ottili-coder używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji ottili-coder.
...
w `~/.local/share/ottili-coder/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim ottili-coder
config.
...
Ottili Coder to lista modeli dostarczonych przez zespół ottili-coder, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z ottili-coder. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Ottili Coder.
:::
...
Działa jak każdy inny dostawca w ottili-coder i jest całkowicie opcjonalny w użyciu.

---

## Ottili Coder Go

Ottili Coder Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół ottili-coder, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z ottili-coder.

1. Uruchom polecenie `/connect` w TUI, wybierz `Ottili Coder Go` i przejdź do [ottili.one/coder/auth](https://ottili.one/coder/auth).
...
Aby używać Amazon Bedrock z ottili-coder:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania ottili-coder:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `ottili-coder.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w ottili-coder nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować ottili-coder do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod ottili-coder działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji ottili-coder.
...
ottili-coder używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować ottili-coder
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`ottili-coder.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem ottili-coder:
...
Aby używać Google Vertex AI z ottili-coder:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania ottili-coder.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą ottili-coder, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację ottili-coder jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja ottili-coder będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować ottili-coder tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować ottili-coder tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować ottili-coder tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem ottili-coder. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/opencode).
:::
...
Aby korzystać z Ollama Cloud z ottili-coder:
...
3. Skopiuj klucz API do użycia w ottili-coder.
...
6. **Ważne**: Przed użyciem modeli chmurowych w ottili-coder musisz pobrać informacje o modelu lokalnie:
...
Ottili Coder to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół ottili-coder. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Ottili Coder</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Ottili Coder**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API ottili-coder.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji ottili-coder.
...
5. Możesz je także dostosować za pomocą konfiguracji ottili-coder. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z ottili-coder:
...
Możesz także dostosować modele za pomocą konfiguracji ottili-coder. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z ottili-coder. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `ottili-coder.json` w katalogu projektu:
...
Pola `limit` pozwalają ottili-coder zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `ottili-coder auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację ottili-coder i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji ottili-coder.
