---
title: Serwer
description: Interakcja z serwerem ottili-coder za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `ottili-coder serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient ottili-coder.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `ottili-coder.local` |
...
Ustaw `OTTILI_CODER_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `ottili-coder` lub ustaw `OTTILI_CODER_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `ottili-coder serve`, jak i `ottili-coder web`.
...
Kiedy uruchomisz `ottili-coder`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera ottili-coder do programowej interakcji z ottili-coder.
:::

Ta architektura umożliwia obsługę wielu klientów przez ottili-coder i programową interakcję z kodem otwartym.

Możesz uruchomić `ottili-coder serve`, aby uruchomić samodzielny serwer. Jeśli masz
ottili-coder TUI działa, `ottili-coder serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki ottili-coder [IDE](/docs/ide).
...
Serwer ottili-coder udostępnia następujące interfejsy API.
