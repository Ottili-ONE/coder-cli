...
Umiejętności agenta pozwalają ottili-coder odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
ottili-coder przeszukuje te lokalizacje:

- Project config: `.ottili-coder/skills/<name>/SKILL.md`
- Global config: `~/.config/ottili-coder/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu ottili-coder przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.ottili-coder/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/ottili-coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.ottili-coder/skills/git-release/SKILL.md` w ten sposób:
...
ottili-coder wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `ottili-coder.json`:
...
**Dla agentów wbudowanych** (w `ottili-coder.json`):
