...
Dzięki ottili-coder możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie ottili-coder używa naszego własnego motywu `ottili-coder`.
...
ottili-coder ma kilka wbudowanych motywów.
...
- Chcą, aby Ottili Coder pasował do wyglądu ich terminala
...
ottili-coder obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/ottili-coder/themes/*.json` lub `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.ottili-coder/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.ottili-coder/themes/*.json`
