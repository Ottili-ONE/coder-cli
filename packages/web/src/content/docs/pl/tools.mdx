...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Ottili Coder posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Ottili Coder.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `OTTILI_CODER_EXPERIMENTAL_LSP_TOOL=true` (lub `OTTILI_CODER_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Ottili Coder lub gdy zmienna środowiskowa `OTTILI_CODER_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Ottili Coder:
