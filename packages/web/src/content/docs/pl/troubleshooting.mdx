...
Aby debugować problemy z ottili-coder, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\ottili-coder\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `ottili-coder --log-level DEBUG`.

---

## Przechowywanie danych

ottili-coder przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/ottili-coder/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\ottili-coder`
...
ottili-coder Desktop uruchamia w tle lokalny serwer ottili-coder (sidecar `ottili-coder-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Ottili Coder` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/ottili-coder/ottili-coder.jsonc` (lub `~/.config/ottili-coder/ottili-coder.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/ottili-coder/ottili-coder.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\ottili-coder\ottili-coder.jsonc`
...
ottili-coder może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/ottili-coder/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\ottili-coder\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby ottili-coder mógł ją odbudować.

1. Całkowicie zamknij ottili-coder Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/ottili-coder`
- **Linux**: usuń `~/.cache/ottili-coder` (lub uruchom `rm -rf ~/.cache/ottili-coder`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\ottili-coder`

3. Uruchom ponownie ottili-coder Desktop.

---

### Napraw problemy z połączeniem z serwerem

ottili-coder Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `ottili-coder.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `OTTILI_CODER_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `OTTILI_CODER_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows ottili-coder Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami ottili-coder.

---

### Brak powiadomień

ottili-coder Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla ottili-coder w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie ottili-coder Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji ottili-coder Desktop):

- `ottili-coder.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `ottili-coder.global.dat` i `ottili-coder.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z ottili-coder:
...
### ottili-coder nie uruchamia się
...
3. Upewnij się, że masz najnowszą wersję z `ottili-coder upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `ottili-coder models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\ottili-coder`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. ottili-coder dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\ottili-coder`

2. Uruchom ponownie kod ottili-coder, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to ottili-coder do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
ottili-coder wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
