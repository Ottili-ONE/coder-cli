...
Existem dois tipos de agentes no ottili-coder; agentes primários e subagentes.
...
ottili-coder vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Ottili Coder vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Ottili Coder vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Ottili Coder, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `ottili-coder.json`:
...
- Global: `~/.config/ottili-coder/agents/`
- Por projeto: `.ottili-coder/agents/`

```markdown title="~/.config/ottili-coder/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o ottili-coder usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do ottili-coder quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do ottili-coder usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Ottili Coder](/docs/zen), você usaria `ottili-coder/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="ottili-coder.json" {3-6,9-12}
{
  "$schema": "https://ottili.one/coder/config.json",
  "tools": {
    "write": true,
...
```json title="ottili-coder.json" {3-5,8-10}
{
  "$schema": "https://ottili.one/coder/config.json",
  "permission": {
    "edit": "deny"
...
```json title="ottili-coder.json" {8}
{
  "$schema": "https://ottili.one/coder/config.json",
  "agent": {
    "build": {
...
```json title="ottili-coder.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Execute `ottili-coder models` para ver uma lista dos modelos disponíveis.
:::
...
```markdown title="~/.config/ottili-coder/agents/docs-writer.md"
---
...
```markdown title="~/.config/ottili-coder/agents/security-auditor.md"
---
