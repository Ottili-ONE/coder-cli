...
O Ottili Coder Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no Ottili Coder. Você assina o Ottili Coder Go e
...
O Ottili Coder Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O Ottili Coder Go funciona como qualquer outro provedor no Ottili Coder.

1. Você entra no **<a href={console}>Ottili Coder</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Ottili Coder Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Ottili Coder Go.
:::
...
O Ottili Coder Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do Ottili Coder
...
Criamos o Ottili Coder Go para:
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Ottili Coder também.
