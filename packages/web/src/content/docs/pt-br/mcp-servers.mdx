...
Você pode adicionar ferramentas externas ao ottili-coder usando o _Model Context Protocol_, ou MCP. O ottili-coder suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do ottili-coder](https://ottili.one/coder/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/ottili-coder`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O ottili-coder lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o ottili-coder irá:
...
Se o servidor requer autenticação, o ottili-coder solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `ottili-coder mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o ottili-coder armazenará os tokens de forma segura em `~/.local/share/ottili-coder/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no ottili-coder, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do ottili-coder como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o ottili-coder à sua conta do Sentry.
