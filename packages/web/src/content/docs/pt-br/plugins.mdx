---
title: Plugins
description: Escreva seus próprios plugins para estender o ottili-coder.
---

Plugins permitem que você estenda o ottili-coder conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do ottili-coder.
...
- `.ottili-coder/plugins/` - Plugins em nível de projeto
- `~/.config/ottili-coder/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/ottili-coder/node_modules/`.
...
1. Configuração global (`~/.config/ottili-coder/ottili-coder.json`)
2. Configuração do projeto (`ottili-coder.json`)
3. Diretório de plugins global (`~/.config/ottili-coder/plugins/`)
4. Diretório de plugins do projeto (`.ottili-coder/plugins/`)
...
O ottili-coder executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do ottili-coder para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o ottili-coder.
...
:::note
Se você estiver usando o aplicativo desktop ottili-coder, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o ottili-coder de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao ottili-coder:
...
O helper `tool` cria uma ferramenta personalizada que o ottili-coder pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o ottili-coder junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://ottili.one/coder/docs/sdk) para detalhes.
