---
title: Servidor
description: Interaja com o servidor ottili-coder via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `ottili-coder serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente ottili-coder pode usar.
...
```bash
ottili-coder serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `ottili-coder.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
ottili-coder serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OTTILI_CODER_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `ottili-coder`, ou defina `OTTILI_CODER_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `ottili-coder serve` quanto ao `ottili-coder web`.

```bash
OTTILI_CODER_SERVER_PASSWORD=your-password ottili-coder serve
```
...
Quando você executa `ottili-coder`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor ottili-coder para interagir com o ottili-coder programaticamente.
:::

Essa arquitetura permite que o ottili-coder suporte múltiplos clientes e permite que você interaja com o ottili-coder programaticamente.

Você pode executar `ottili-coder serve` para iniciar um servidor autônomo. Se você tiver o TUI do ottili-coder em execução, `ottili-coder serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do ottili-coder [IDE](/docs/ide).
...
O servidor ottili-coder expõe as seguintes APIs.
