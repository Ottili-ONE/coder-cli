...
As habilidades do agente permitem que o ottili-coder descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O ottili-coder pesquisa nesses locais:

- Configuração do projeto: `.ottili-coder/skills/<name>/SKILL.md`
- Configuração global: `~/.config/ottili-coder/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o ottili-coder sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.ottili-coder/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/ottili-coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.ottili-coder/skills/git-release/SKILL.md` assim:
...
O ottili-coder lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `ottili-coder.json`:
...
**Para agentes embutidos** (em `ottili-coder.json`):
