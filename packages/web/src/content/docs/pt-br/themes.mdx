...
Com o ottili-coder, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o ottili-coder usa nosso próprio tema `ottili-coder`.
...
O ottili-coder vem com vários temas embutidos.
...
- Querem que o ottili-coder corresponda à aparência do seu terminal
...
O ottili-coder suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/ottili-coder/themes/*.json` ou `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.ottili-coder/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.ottili-coder/themes/*.json`
