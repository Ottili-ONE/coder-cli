...
Para depurar problemas com o ottili-coder, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\ottili-coder\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `ottili-coder --log-level DEBUG`.

---

## Armazenamento

O ottili-coder armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/ottili-coder/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\ottili-coder`
...
O ottili-coder Desktop executa um servidor ottili-coder local (o sidecar `ottili-coder-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `Ottili Coder` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/ottili-coder/ottili-coder.jsonc` (ou `~/.config/ottili-coder/ottili-coder.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/ottili-coder/ottili-coder.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\ottili-coder\ottili-coder.jsonc`
...
O ottili-coder também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/ottili-coder/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\ottili-coder\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o ottili-coder possa reconstruí-lo.

1. Saia completamente do ottili-coder Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/ottili-coder`
- **Linux**: exclua `~/.cache/ottili-coder` (ou execute `rm -rf ~/.cache/ottili-coder`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\ottili-coder`

3. Reinicie o ottili-coder Desktop.

---

### Corrigindo problemas de conexão com o servidor

O ottili-coder Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `ottili-coder.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `OTTILI_CODER_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `OTTILI_CODER_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o ottili-coder Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do ottili-coder.

---

### Notificações não aparecendo

O ottili-coder Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o ottili-coder nas configurações do seu sistema operacional, e
...
1. Saia do ottili-coder Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo ottili-coder Desktop):

- `ottili-coder.settings.dat` (URL do servidor padrão do desktop)
- `ottili-coder.global.dat` e `ottili-coder.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o ottili-coder:
...
Para descobrir quais modelos você tem acesso, execute `ottili-coder models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\ottili-coder`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O ottili-coder instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\ottili-coder`

2. Reinicie o ottili-coder para reinstalar os pacotes de provedores mais recentes

Isso forçará o ottili-coder a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O ottili-coder detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
