...
Создайте `.ottili-coder/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию ottili-coder или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем ottili-coder [config](/docs/config):
...
- Глобальный: `~/.config/ottili-coder/commands/`
- Для каждого проекта: `.ottili-coder/commands/`
...
ottili-coder включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
