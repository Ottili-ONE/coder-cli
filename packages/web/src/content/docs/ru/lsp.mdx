---
title: LSP-серверы
description: ottili-coder интегрируется с вашими серверами LSP.
---

ottili-coder может интегрироваться с серверами Language Server Protocol (LSP), чтобы использовать диагностику как обратную связь для агента.

---

## Встроенные

ottili-coder поставляется с несколькими встроенными LSP-серверами для популярных языков:
...
:::note
Вы можете отключить автоматическую загрузку LSP-сервера, установив для переменной среды `OTTILI_CODER_DISABLE_LSP_DOWNLOAD` значение `true`.
:::
...
Когда LSP включен и ottili-coder открывает файл, он:
...
Вы можете включить и настроить серверы LSP через раздел `lsp` в конфигурации ottili-coder.
