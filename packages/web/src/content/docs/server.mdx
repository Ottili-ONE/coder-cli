---
title: Server
description: Interact with ottili-coder server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `ottili-coder serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an ottili-coder client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `ottili-coder.local` |
...
Set `OTTILI_CODER_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `ottili-coder`, or set `OTTILI_CODER_SERVER_USERNAME` to override it. This applies to both `ottili-coder serve` and `ottili-coder web`.
...
When you run `ottili-coder` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the ottili-coder server to interact with ottili-coder programmatically.
:::

This architecture lets ottili-coder support multiple clients and allows you to interact with ottili-coder programmatically.

You can run `ottili-coder serve` to start a standalone server. If you have the
ottili-coder TUI running, `ottili-coder serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Ottili Coder [IDE](/docs/ide) plugins.
...
The ottili-coder server exposes the following APIs.
