...
Tools allow the LLM to perform actions in your codebase. Ottili Coder comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Ottili Coder.
...
:::note
This tool is only available when `OTTILI_CODER_EXPERIMENTAL_LSP_TOOL=true` (or `OTTILI_CODER_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the Ottili Coder provider or when the `OTTILI_CODER_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching Ottili Coder:
