...
ottili-coder'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
ottili-coder, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Ottili Coder, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Ottili Coder iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `ottili-coder.json` yapılandırma dosyanızda yapılandırın:
...
- Proje başına: `.ottili-coder/agents/`
...
Sıcaklık belirtilmezse ottili-coder modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global ottili-coder yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
ottili-coder yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Ottili Coder](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `ottili-coder/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `ottili-coder models` komutunu çalıştırın.
:::
