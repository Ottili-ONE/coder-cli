...
Ottili Coder Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Ottili Coder'daki diğer sağlayıcılar gibi çalışır. Ottili Coder Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Ottili Coder'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Ottili Coder Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Ottili Coder Go, Ottili Coder'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Ottili Coder</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Ottili Coder Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Ottili Coder Go'ya abone olabilir.
:::
...
Ottili Coder Go aşağıdaki limitleri içerir:
...
Ottili Coder yapılandırmanızdaki [model id](/docs/config/#models) formatı `opencode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `opencode-go/kimi-k2.6` kullanmalısınız.
...
Ottili Coder Go'yu şu amaçlarla oluşturduk:
...
4. Ottili Coder ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
