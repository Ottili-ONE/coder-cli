...
_Model Bağlam Protokolü_ veya MCP kullanarak ottili-coder'a harici araçlar ekleyebilirsiniz. ottili-coder hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [ottili-coder Config](https://ottili.one/coder/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/ottili-coder` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
ottili-coder, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde ottili-coder şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa ottili-coder, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `ottili-coder mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından ottili-coder, belirteçleri `~/.local/share/ottili-coder/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra ottili-coder'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi ottili-coder yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve ottili-coder'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
