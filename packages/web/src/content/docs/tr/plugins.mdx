---
title: Eklentiler
description: Ottili Coder'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Ottili Coder'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Ottili Coder'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.ottili-coder/plugins/` - Proje düzeyinde eklentiler
- `~/.config/ottili-coder/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/ottili-coder/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/ottili-coder/ottili-coder.json`)
2. Project config (`ottili-coder.json`)
3. Global eklenti dizini (`~/.config/ottili-coder/plugins/`)
4. Proje eklenti dizini (`.ottili-coder/plugins/`)
...
Ottili Coder bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Ottili Coder SDK istemcisi.
...
Ottili Coder'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Ottili Coder masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Ottili Coder'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Ottili Coder'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Ottili Coder'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Ottili Coder'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://ottili.one/coder/docs/sdk) bakın.
