---
title: Sağlayıcılar
description: ottili-coder'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

ottili-coder, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı ottili-coder yapılandırmanızda yapılandırın.
...
`~/.local/share/ottili-coder/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları ottili-coder'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Ottili Coder, ottili-coder ekibi tarafından sağlanan modellerin bir listesidir.
ottili-coder ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Ottili Coder ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, ottili-coder'u seçin ve [ottili.one/coder/auth](https://ottili.one/coder/auth)'ye gidin.
...
ottili-coder'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Ottili Coder Go

Ottili Coder Go, ottili-coder ile iyi çalıştığı test edilmiş ve doğrulanmış, ottili-coder ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Ottili Coder Go`'yu seçin ve [ottili.one/coder/auth](https://ottili.one/coder/auth) adresine gidin.
...
Amazon Bedrock'u ottili-coder ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   ottili-coder'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `ottili-coder.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi ottili-coder'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
ottili-coder'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   ottili-coder'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
ottili-coder, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. ottili-coder'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`ottili-coder.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`ottili-coder.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi ottili-coder'la kullanmak için:
...
Google Vertex AI'yi ottili-coder ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları ottili-coder çalıştırılırken ayarlayın.
...
Helicone'un ottili-coder aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her ottili-coder görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her ottili-coder görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
ottili-coder'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla ottili-coder'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla ottili-coder'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini ottili-coder için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/opencode)'a bakın.
:::
...
Ollama Cloud'u ottili-coder ile kullanmak için:
...
3. ottili-coder'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: ottili-coder'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Ottili Coder, ottili-coder ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Ottili Coder</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Ottili Coder**'i arayın.

   ```txt
   /connect
   ```

3. ottili-coder API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   ottili-coder yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca ottili-coder yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri ottili-coder yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı ottili-coder ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `ottili-coder.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları ottili-coder'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `ottili-coder auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için ottili-coder yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, ottili-coder yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
