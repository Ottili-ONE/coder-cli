---
title: Sunucu
description: ottili-coder sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`ottili-coder serve` komutu, ottili-coder istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `ottili-coder.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `OTTILI_CODER_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `ottili-coder` değeridir; değiştirmek isterseniz `OTTILI_CODER_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `ottili-coder serve` hem de `ottili-coder web` için geçerlidir.
...
`ottili-coder` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
ottili-coder ile programatik etkileşim için ottili-coder sunucusunu kullanın.
:::

Bu mimari, ottili-coder'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`ottili-coder serve` ile bağımsız bir sunucu başlatabilirsiniz. ottili-coder TUI açıksa, `ottili-coder serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum ottili-coder [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
ottili-coder sunucusu aşağıdaki API'leri sunar.
