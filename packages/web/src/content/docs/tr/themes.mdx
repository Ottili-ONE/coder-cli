...
ottili-coder ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak ottili-coder kendi `ottili-coder` temasını kullanır.
...
ottili-coder birden fazla yerleşik temayla gelir.
...
- ottili-coder'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
ottili-coder, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/ottili-coder/themes/*.json` veya `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.ottili-coder/themes/*.json`
4. **Geçerli çalışma dizini** - `./.ottili-coder/themes/*.json`
