---
title: TUI
description: Using the Ottili Coder terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Ottili Coder provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Ottili Coder starts the TUI for the current directory.
...
When using the Ottili Coder TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Ottili Coder. Allows you to select from available providers and add their API keys.
...
Exit Ottili Coder. _Aliases_: `/quit`, `/q`
...
This is separate from `ottili-coder.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Ottili Coder waits after the leader key. Defaults to `2000`.
...
Use `OTTILI_CODER_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `ottili-coder.default`.
