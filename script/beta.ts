...
const model = "ottili-coder/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/ottili-coder")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with ottiliCoder...`)
  ...
  try {
    await $`ottili-coder run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  ottili-coder failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with ottili-coder")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with ottiliCoder...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/ottili-coder`.",
