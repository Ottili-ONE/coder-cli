...
import { createOttiliCoder } from "@opencode-ai/sdk"
...
`)
  ...
  const ottiliCoder = await createOttiliCoder({ port: 0 })
  ...
  try {
    ...
    const session = await ottiliCoder.client.session.create()
    const result = await ottiliCoder.client.session
      .prompt({
