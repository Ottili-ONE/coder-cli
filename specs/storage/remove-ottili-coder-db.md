# Remove `packages/ottili-coder/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/ottili-coder/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the ottili-coder legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/ottili-coder/src/storage/db.ts` are concentrated in 22 source files:

- `packages/ottili-coder/src/account/repo.ts`
- `packages/ottili-coder/src/cli/cmd/db.ts`
- `packages/ottili-coder/src/cli/cmd/import.ts`
- `packages/ottili-coder/src/cli/cmd/stats.ts`
- `packages/ottili-coder/src/control-plane/workspace.ts`
- `packages/ottili-coder/src/index.ts`
- `packages/ottili-coder/src/node.ts`
- `packages/ottili-coder/src/permission/index.ts`
- `packages/ottili-coder/src/project/project.ts`
- `packages/ottili-coder/src/server/projectors.ts`
- `packages/ottili-coder/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/ottili-coder/src/server/shared/fence.ts`
- `packages/ottili-coder/src/session/message-v2.ts`
- `packages/ottili-coder/src/session/projectors.ts`
- `packages/ottili-coder/src/session/prompt.ts`
- `packages/ottili-coder/src/session/session.ts`
- `packages/ottili-coder/src/session/todo.ts`
- `packages/ottili-coder/src/share/share-next.ts`
- `packages/ottili-coder/src/storage/db.ts`
- `packages/ottili-coder/src/sync/index.ts`
- `packages/ottili-coder/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy ottili-coder database wrapper; `packages/ottili-coder/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the ottili-coder wrapper.
...
Status: Completed. `SyncEvent` and the ottili-coder projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy ottili-coder database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy ottili-coder database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy ottili-coder database wrapper.
...
All migration groups are complete or superseded. `packages/ottili-coder/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No ottili-coder data-migration group remains.

The previous ottili-coder `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate ottili-coder data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/ottili-coder`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/ottili-coder/src`
- `bun typecheck` from `packages/ottili-coder`
- Relevant package tests from `packages/ottili-coder`, not the repo root
