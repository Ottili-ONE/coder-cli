...
Move the canonical Ottili Coder terminal application from
`packages/ottili-coder/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/tui
name: @ottili-coder/tui
```

Target dependency graph:

```text
packages/ottili-coder ---\
                      > @ottili-coder/tui -> @ottili-coder/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/ottili-coder`,
`packages/cli`, or `@ottili-coder/core`.

The SDK is the TUI's Ottili Coder boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/ottili-coder`.
- Do not replace private `packages/ottili-coder` imports with `@ottili-coder/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@ottili-coder/tui`.

## Ownership Boundary

### `@ottili-coder/tui` Owns
...
- Ottili Coder domain data displayed by the TUI
...
```text
packages/ottili-coder/src/cli/cmd/tui
```

Its private dependency on `packages/ottili-coder` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/ottili-coder/src/*`.
...
Status: Completed. The private `@ottili-coder/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@ottili-coder/tui`.
...
- The package has no dependency on `ottili-coder`, `@ottili-coder/cli`, or
  `@ottili-coder/core`.
...
primitives now live in `@ottili-coder/tui`. The legacy host consumes explicit
...
and build inputs through `@ottili-coder/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Ottili Coder globals or process state;
...
Replace process-global Ottili Coder state with resolved TUI inputs.
...
- Keep environment reads needed by legacy command or worker startup in
  `packages/ottili-coder` adapters.
...
`@ottili-coder/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/ottili-coder` initially.
...
canonical in `@ottili-coder/tui`. Configured references resolve through the new
...
adapters rather than Ottili Coder domain implementations.

Make the SDK the only Ottili Coder domain boundary used by the TUI.
...
- Domain-facing TUI code imports Ottili Coder data and operations only from
  `@ottili-coder/sdk`.
...
`@ottili-coder/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/ottili-coder`.
...
- The package has no imports from `packages/ottili-coder`, `packages/cli`, or
  `@ottili-coder/core`.
...
public `@ottili-coder/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/ottili-coder`.
- Keep the legacy embedded worker and server startup in `packages/ottili-coder`.
- Change those adapters to load config, create transport inputs, and call the
  public `@ottili-coder/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/ottili-coder`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@ottili-coder/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@ottili-coder/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Ottili Coder domain state.
...
cd packages/ottili-coder && bun typecheck
...
rg '@ottili-coder/core|packages/ottili-coder|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/ottili-coder/src packages/ottili-coder/test
```
...
- Build the current-platform `packages/ottili-coder` binary.
