...
Both `action` and `resource` use ottili-coder's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.ottili-coder` files is intentionally deferred until `.ottili-coder` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether ottili-coder permits an operation through its managed execution paths.
