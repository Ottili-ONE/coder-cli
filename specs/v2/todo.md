...
The ottili-coder server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@ottili-coder/llm` tool loop after replacing its
...
- plugins get global "ottili-coder" instance like in that post i showed
- ottili-coder instance has stuff like `ottili-coder.session.prompt()` or
  `ottili-coder.tool.register({...})`
...
- decide whether to preserve deprecated `@ottili-coder/llm` orchestration exports
